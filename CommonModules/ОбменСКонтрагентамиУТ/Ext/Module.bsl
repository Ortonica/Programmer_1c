...
Процедура РСК_ЗаполнитьДанныеПервичногоДокументаУПД_2019(СтруктураДанных, СтруктураЭД, ДеревоДанных, Настройки, Отказ)
	...
	#Вставка
	//+РС Консалт Назаров М.Ю. 30.03.2023 12:16:31
	ГрузополучательОнЖе = ДанныеШапки.Грузополучатель = ДанныеШапки.Контрагент;
	//-РС Консалт Назаров М.Ю. 30.03.2023 12:16:31
	#КонецВставки
	...
	Если ЭтоЭлектронноеАктированиеЕИС И ТипЗнч(ДокументОтгрузки) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		Если ЗначениеЗаполнено(ДокументОтгрузки.АдресДоставкиЗначенияПолей) Тогда
			...
			ЭлектронноеАктированиеЕИСУТ.ЗаполнитьМестаПоставкиПриложенияЕИСПоАдресуДоставки(
				...
				#Вставка
				//+РС Консалт Назаров М.Ю. 30.03.2023 12:17:55
				// Если грузополучатель и покупатель одно лицо, то Идентификатор участника будет "0"
				// Если грузополучатель отличается от покупателя, то в местах поставки будет ошибка, если указать "0", 
				// так как места поставки должны принадлежать грузополучателю, то есть "1"
				Если Не ГрузополучательОнЖе Тогда 
					Если ДанныеПриложенияЕИС.МестаПоставки.Количество() = 1 Тогда 
						ДанныеПриложенияЕИС.МестаПоставки[0].ИдентификаторУчастника = ЭлектронноеАктированиеЕИС.НачальныйИндексИнформацииУчастникаГрузополучателя();										
					КонецЕсли;
				КонецЕсли;
				//-РС Консалт Назаров М.Ю. 30.03.2023 12:17:55
				#КонецВставки
	...
	Если ЭтоЭлектронноеАктированиеЕИС И ТипЗнч(ДокументОтгрузки) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда 
		...
		// Указание дат по началу и окончанию поставки товаров
		...
		//+РС Консалт Назаров М.Ю. 04.04.2023 6:19:54
		// Указание данных о транспортировке   
		Если ЗначениеЗаполнено(ДокументОтгрузки.РСК_ДанныеОТранспортировке) Тогда 
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных, "СведенияОТранспортировке", ДокументОтгрузки.РСК_ДанныеОТранспортировке);
		КонецЕсли;  
		//-РС Консалт Назаров М.Ю. 04.04.2023 6:19:54
	...
	Если ТипЗнч(ДокументОтгрузки) <> Тип("ДокументСсылка.КорректировкаРеализации")
			...
			//-- Устарело_Переработка24
			и ТипЗнч(ДокументОтгрузки) <> Тип("ДокументСсылка.ОтчетДавальцу2_5")
			//-- НЕ УТКА
			и ТипЗнч(ДокументОтгрузки) <> Тип("ДокументСсылка.ВозвратТоваровПоставщику")
			И ТипЗнч(ДокументОтгрузки) <> Тип("ДокументСсылка.ВыкупТоваровХранителем")
	...
	Если Не ВидСчетаФактуры = "Авансовый" Тогда
		...
		Если ЗначениеЗаполнено(ДанныеШапки.Основание)
			...
			#Вставка
			//+РС Консалт Назаров М.Ю. 20.02.2023 11:41:56
			// Попросили выгружать дополнительные сведения такой строкой
			Если ЭтоЭлектронноеАктированиеЕИС 
				И ТипЗнч(Настройки) = Тип("Структура") 
				И Настройки.Свойство("Договор")
				И ЗначениеЗаполнено(Настройки.Договор) 
				И ЗначениеЗаполнено(Настройки.Договор.ГосударственныйКонтракт) Тогда  
				
				РеестровыйНомерКонтракта = СокрЛП(Настройки.Договор.ГосударственныйКонтракт.РеестровыйНомерКонтракта);
				Если ЗначениеЗаполнено(РеестровыйНомерКонтракта) Тогда 
					НоваяСтрока.ДокументДопСведения = СтрШаблон("Реестровый номер в реестре контрактов: %1", РеестровыйНомерКонтракта);
				КонецЕсли;
...
Процедура РСК_ЗаполнитьСведенияОТаможеннойДекларации(ПриемникДанныхТовара, ИсточникДанныхТовара)
	...
	КонецЕсли;   
	#Вставка
	//+РС Консалт Назаров М.Ю. 18.04.2023 9:41:27 
	// Даже если ГТД говорит, что товар из Китая, все равно необходимо проверить на Российское происхождение и не писать ГТД
	Если ЭтоРоссийскийТовар(ИсточникДанныхТовара) Тогда 
		Возврат;	
	КонецЕсли;
	//-РС Консалт Назаров М.Ю. 18.04.2023 9:41:27
	#КонецВставки
...
Функция ЭтоРоссийскийТовар(Знач ИсточникДанныхТовара)
	
	ЭтоРоссийскийТовар = Ложь;
	
	Если ЗначениеЗаполнено(ИсточникДанныхТовара.НоменклатураПартнера) Тогда 
		НаименованиеНоменклатуры = НРег(ИсточникДанныхТовара.НоменклатураПартнера.НаименованиеПолное);
		ЭтоРоссийскийТовар = СтрНайти(НаименованиеНоменклатуры, "российская федерация") <> 0;
	КонецЕсли;
	
	Возврат ЭтоРоссийскийТовар;
