...
Процедура РСК_ПроверитьКорректностьЗаполненияДокументаПродажи(Знач ДокументПродажи, Отказ)
	...
	ИначеЕсли ТипДокумента = Тип("ДокументОбъект.ЗаказДавальца") Тогда
		...
		ИспользоватьСкидки                          = Ложь;
	//-- Устарело_Переработка24
	ИначеЕсли ТипДокумента = Тип("ДокументОбъект.ЗаказДавальца2_5") Тогда
		
		ВсеСтрокиОтменены = ОбщегоНазначенияУТ.ВсеСтрокиОтменены(ДокументПродажи, "Продукция", "Отменено");
		ВсеСтрокиОтменены =
			ВсеСтрокиОтменены И ОбщегоНазначенияУТ.ВсеСтрокиОтменены(ДокументПродажи, "Материалы", "Отменено");
		
		НужноСогласовать                            = Не (ВсеСтрокиОтменены Или ОтклонениеОтУсловийПродаж);
		НужноСогласоватьРучныеСкидкиПоПользователям = Ложь;
		ИспользоватьСкидки                          = Ложь;
	//-- НЕ УТКА
	...
	КонтролироватьРасчетыДавальцев2_5			= ПолучитьФункциональнуюОпцию("НеЗакрыватьЗаказыДавальцевБезПолнойОплаты2_5");
	КонтрольОтгрузкиЗаказаДавальца2_5			= ПолучитьФункциональнуюОпцию("НеЗакрыватьЗаказыДавальцевБезПолнойОтработки2_5");
	ИспользоватьСтатусыЗаказовДавальцев2_5		= ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыЗаказовДавальцев2_5");
	//-- НЕ УТКА
	...
	ИначеЕсли ИмяТаблицы = "Документ.ЗаказДавальца" Тогда
		...
		КонецЕсли;
	//-- Устарело_Переработка24
	ИначеЕсли ИмяТаблицы = "Документ.ЗаказДавальца2_5" Тогда
		...
		Если ДокументПродажи.Статус = Перечисления.СтатусыЗаказовДавальцев.Закрыт И Не ВсеСтрокиОтменены Тогда
			...
			Если ДокументПродажи.ПорядокРасчетов <> Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов
			   И КонтролироватьРасчетыДавальцев2_5
			   И ИспользоватьСтатусыЗаказовДавальцев2_5 Тогда
				МассивПроверок.Добавить("КонтрольЗавершенияРасчетов");
			КонецЕсли;
			
			Если КонтрольОтгрузкиЗаказаДавальца2_5 И ИспользоватьСтатусыЗаказовДавальцев2_5 Тогда
				МассивПроверок.Добавить("КонтрольЗавершенияОтгрузки2_5");
				МассивПроверок.Добавить("КонтрольОформленияОтчетов2_5");
			КонецЕсли;
	...
	ИначеЕсли ИмяТаблицы = "Документ.КорректировкаРеализации" Тогда
		...
		//++ НЕ УТКА
		Если Не ТипЗнч(ДокументПродажи.ДокументОснование) = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
		//-- НЕ УТКА
		МассивПроверок.Добавить("КонтрольЗаполненияЦен");
		МассивПроверок.Добавить("КорректностьСуммыИСуммыНДС");
		//++ НЕ УТКА
	...
	Для Каждого ТекЭлемент Из МассивПроверок Цикл
		...
		ИначеЕсли ТекЭлемент = "КонтрольЗавершенияОтгрузки2_5" Тогда
			
			СформироватьЗапросКонтрольЗавершенияОтгрузки2_5(ТекстЗапроса);
		...
		//-- Устарело_Переработка24
		ИначеЕсли ТекЭлемент = "КонтрольОформленияОтчетов2_5" Тогда
		
			СформироватьЗапросКонтрольОформленияОтчетов2_5(ТекстЗапроса)
	...
	Для ТекИндекс = 0 По МассивРезультатов.Количество()-1 Цикл
		...
		ИначеЕсли МассивПроверок[ТекИндекс] = "КонтрольЗавершенияОтгрузки2_5" Тогда
		...
		ИначеЕсли МассивПроверок[ТекИндекс] = "КонтрольОформленияОтчетов2_5" Тогда
...
Процедура РСК_СоздатьДокументПродажи(ПоляДокумента, ПараметрыОснования, МассивДокументов, ПараметрыФормирования)
	...
	//++ НЕ УТКА
	ИначеЕсли ПоляДокумента.ХозяйственнаяОперация =  Перечисления.ХозяйственныеОперации.ПередачаДавальцу2_5 Тогда
	...
	Если ПоляДокумента.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ВозвратТоваровОтКлиента
		...
		//++ НЕ УТКА
		И ПоляДокумента.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПередачаДавальцу2_5
		//-- НЕ УТКА
	...
	//++ НЕ УТКА
	 Или ПоляДокумента.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаДавальцу2_5
	//-- НЕ УТКА
	Тогда
	...
	Если НовыйДокумент.ПроверитьЗаполнение() Тогда
		...
		//++ НЕ УТКА
		ПровестиДокумент = ПровестиДокумент
						   Или ПоляДокумента.ХозяйственнаяОперация =  Перечисления.ХозяйственныеОперации.ПередачаДавальцу2_5;
		//-- НЕ УТКА
		...
		Если ПоляДокумента.НалогообложениеНДС <> Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС
			...
			И Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НовыйДокумент.Контрагент, "ЮрФизЛицо") = Перечисления.ЮрФизЛицо.ФизЛицо
			И (НовыйДокумент.Ссылка.Метаданные().Реквизиты.Найти("Партнер") = Неопределено
			Или Не НовыйДокумент.Партнер = Справочники.Партнеры.РозничныйПокупатель)
			//++ РС Консалт: Минаков А.С. Задача 245086
			#КонецВставки
...
Функция РСК_ТекстЗапросаПоЗначащимРеквизитамРаспоряжений()
	...
	//++ НЕ УТКА
	ПравоДоступаЗаказДавальца2_5 = ПравоДоступа("Чтение", Метаданные.Документы.ЗаказДавальца2_5);
	
	Если ПравоДоступаЗаказДавальца2_5 Тогда
		
		ТекстыЗапросаВТЗаказы.Добавить(
		"ВЫБРАТЬ
		|	ЗаказДавальца2_5Продукция.Ссылка КАК Ссылка,
		|	ЗаказДавальца2_5Продукция.Склад КАК Склад
		|ИЗ
		...
		|	ПО ЗаказДавальца2_5.Ссылка = ЗаказДавальца2_5Продукция.Ссылка
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
		|	ПО Склады.Ссылка = ЗаказДавальца2_5.Склад
		|
		|ГДЕ ЗаказДавальца2_5Продукция.Ссылка В (&МассивСсылок)
		|	И Склады.ЭтоГруппа
		|	И Склады.ВыборГруппы = ЗНАЧЕНИЕ(Перечисление.ВыборГруппыСкладов.РазрешитьВЗаказах)
		|	И (НЕ &ИспользоватьОтборПоСкладу
		|		ИЛИ ЗаказДавальца2_5Продукция.Склад В(&Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)))
		|");
		...
		|	ЗаказДавальца2_5.ТребуетсяЗалогЗаТару                  КАК ТребуетсяЗалогЗаТару,
		...
		|	ЗаказДавальца2_5.АдресДоставкиПеревозчика              КАК АдресДоставкиПеревозчика
		|ПОМЕСТИТЬ ВтЗаказыКлиентов
		|ИЗ
		...
		|	
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца2_5 КАК ЗаказДавальца2_5
		|	ПО ЗаказДавальца2_5.Ссылка = ВТЗаказов.Ссылка
		|";
		
		Если ПравоДоступаЗаказКлиента Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РАЗРЕШЕННЫЕ", ""); //@Query-part-1
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПОМЕСТИТЬ ВтЗаказыКлиентов", ""); //@Query-part-1
		КонецЕсли;
		
		ТекстыЗапросаЗаказы.Добавить(ТекстЗапроса);
	...
	ТекстыРаспоряжениеНаОформление.Добавить(
		...
		|		ВЫБОР
		|			КОГДА ТИПЗНАЧЕНИЯ(ЗаказыКлиентовОстатки.ЗаказКлиента) = ТИП(Документ.ЗаказКлиента)
		|				ТОГДА ВЫРАЗИТЬ(ЗаказыКлиентовОстатки.ЗаказКлиента КАК Документ.ЗаказКлиента).Соглашение
		|			КОГДА ТИПЗНАЧЕНИЯ(ЗаказыКлиентовОстатки.ЗаказКлиента) = ТИП(Документ.ЗаявкаНаВозвратТоваровОтКлиента)
		|				ТОГДА ВЫРАЗИТЬ(ЗаказыКлиентовОстатки.ЗаказКлиента КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).Соглашение
		...
		//++ НЕ УТКА
		|			КОГДА ЗаказыКлиентовОстатки.ЗаказКлиента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья2_5)
		|				ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		//-- НЕ УТКА
		|			ИНАЧЕ ЗаказыКлиентовОстатки.ЗаказКлиента.ХозяйственнаяОперация
		|		КОНЕЦ КАК ХозяйственнаяОперация,
		|		ВЫБОР
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ЗаказыКлиентовОстатки.ЗаказКлиента) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ЗаказыКлиентовОстатки.КПередачеОстаток
		//-- НЕ УТКА
		|			КОГДА ИСТИНА
		|				ТОГДА ЗаказыКлиентовОстатки.КОформлениюОстаток
	...
	#КонецВставки
		...
		//-- НЕ УТКА
		|			КОГДА ИСТИНА
		|				ТОГДА ЗаказыКлиентовОстатки.ЗаказКлиента.ВернутьМногооборотнуюТару
		...
		//-- НЕ УТКА
		|			КОГДА ИСТИНА
		|				ТОГДА ЗаказыКлиентовОстатки.ЗаказКлиента.СрокВозвратаМногооборотнойТары
		...
		|				ТОГДА ВЫРАЗИТЬ(ЗаказыКлиентовОстатки.ЗаказКлиента КАК Документ.ЗаказКлиента).Соглашение.КалендарьВозвратаТары
		|			КОГДА &ИспользоватьСоглашенияСКлиентами
		|				И ТИПЗНАЧЕНИЯ(ЗаказыКлиентовОстатки.ЗаказКлиента) = ТИП(Документ.ЗаявкаНаВозвратТоваровОтКлиента)
		|				ТОГДА ВЫРАЗИТЬ(ЗаказыКлиентовОстатки.ЗаказКлиента КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).Соглашение.КалендарьВозвратаТары
		|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПроизводственныеКалендари.ПустаяСсылка)
		|		КОНЕЦ КАК КалендарьВозвратаТары,
		|		ВЫБОР
		|			КОГДА &ИспользоватьСоглашенияСКлиентами
		|				И ТИПЗНАЧЕНИЯ(ЗаказыКлиентовОстатки.ЗаказКлиента) = ТИП(Документ.ЗаказКлиента)
		|				ТОГДА ВЫРАЗИТЬ(ЗаказыКлиентовОстатки.ЗаказКлиента КАК Документ.ЗаказКлиента).Соглашение.РассчитыватьДатуВозвратаТарыПоКалендарю
		|			КОГДА &ИспользоватьСоглашенияСКлиентами
		|				И ТИПЗНАЧЕНИЯ(ЗаказыКлиентовОстатки.ЗаказКлиента) = ТИП(Документ.ЗаявкаНаВозвратТоваровОтКлиента)
		|				ТОГДА ВЫРАЗИТЬ(ЗаказыКлиентовОстатки.ЗаказКлиента КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).Соглашение.РассчитыватьДатуВозвратаТарыПоКалендарю
		|			ИНАЧЕ ЛОЖЬ
		...
		|					И ВЫБОР
		//++ НЕ УТКА
		|						КОГДА ТИПЗНАЧЕНИЯ(ЗаказКлиента) = ТИП(Документ.ЗаказДавальца2_5)
		|							ТОГДА ИСТИНА
		//-- НЕ УТКА
		|						КОГДА ИСТИНА
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).Партнер
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).Партнер
		//-- НЕ УТКА
		|		КОНЕЦ КАК Партнер,
		|		ВЫБОР
		|			КОГДА ТоварыКОтгрузкеОстатки.ДокументОтгрузки ССЫЛКА Документ.РеализацияТоваровУслуг ТОГДА
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).Контрагент
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).Контрагент
		//-- НЕ УТКА
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).Договор
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).Договор
		//-- НЕ УТКА
		|		КОНЕЦ КАК Договор,
		|		ВЫБОР
		|			КОГДА ТоварыКОтгрузкеОстатки.ДокументОтгрузки ССЫЛКА Документ.РеализацияТоваровУслуг ТОГДА
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).Организация
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).Организация
		//-- НЕ УТКА
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).Сделка
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).Сделка
		//-- НЕ УТКА
		|		КОНЕЦ КАК Сделка,
		|		ВЫБОР
		|			КОГДА ТоварыКОтгрузкеОстатки.ДокументОтгрузки ССЫЛКА Документ.РеализацияТоваровУслуг ТОГДА
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).Склад
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).Склад
		//-- НЕ УТКА
		|		КОНЕЦ КАК Склад,
		|		ВЫБОР
		|			КОГДА ТоварыКОтгрузкеОстатки.ДокументОтгрузки ССЫЛКА Документ.РеализацияТоваровУслуг ТОГДА
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).Подразделение
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).Подразделение
		//-- НЕ УТКА
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).Склад.ЭтоГруппа
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).Склад.ВыборГруппы =
		|					ЗНАЧЕНИЕ(Перечисление.ВыборГруппыСкладов.РазрешитьВЗаказах)
		|				ТОГДА ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).Склад.ЭтоГруппа
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).Склад.ВыборГруппы =
		|					ЗНАЧЕНИЕ(Перечисление.ВыборГруппыСкладов.РазрешитьВЗаказах)
		|				ТОГДА ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		//-- НЕ УТКА
		|			ИНАЧЕ ВЫБОР
		...
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация
		//-- НЕ УТКА
		|			ИНАЧЕ ВЫБОР
		|				КОГДА ТоварыКОтгрузкеОстатки.ДокументОтгрузки ССЫЛКА Документ.РеализацияТоваровУслуг ТОГДА
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).Валюта
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).Валюта
		//-- НЕ УТКА
		|		КОНЕЦ КАК Валюта,
		|		ВЫБОР
		|			КОГДА ТоварыКОтгрузкеОстатки.ДокументОтгрузки ССЫЛКА Документ.РеализацияТоваровУслуг ТОГДА
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).НалогообложениеНДС
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).ЦенаВключаетНДС
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).СпособДоставки
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).СпособДоставки
		//-- НЕ УТКА
	...
	#КонецВставки
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).СпособДоставки В (
		|											ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчика),
		|											ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчикаПоАдресу))
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).ПеревозчикПартнер
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).СпособДоставки В (
		|											ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчика),
		|											ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчикаПоАдресу))
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ПеревозчикПартнер
		//-- НЕ УТКА
		|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)
		|		КОНЕЦ,
		|		ВЫБОР
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				И (ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).СпособДоставки <> ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)
		|					ИЛИ ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).АдресДоставки <> """")
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).АдресДоставки
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				И (ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).СпособДоставки <> ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)
		|					ИЛИ ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).АдресДоставки <> """")
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).АдресДоставки
		//-- НЕ УТКА
		|			ИНАЧЕ """"
		|		КОНЕЦ,
		|		ВЫБОР
	...
	#КонецВставки
		...
		|											ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчика),
		|											ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчикаПоАдресу))
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).АдресДоставкиПеревозчика
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		...
		|											ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчика),
		|											ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчикаПоАдресу))
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).АдресДоставкиПеревозчика
		//-- НЕ УТКА
		|			ИНАЧЕ """"
		|		КОНЕЦ,
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).ТребуетсяЗалогЗаТару
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).НаправлениеДеятельности
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).НаправлениеДеятельности
		//-- НЕ УТКА
		...
		//++ НЕ УТКА
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ЗаказДавальца2_5)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ЗаказДавальца2_5).ПорядокРасчетов
		|				
		|			КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки) = ТИП(Документ.ОтгрузкаТоваровСХранения)
		|				И ВЫРАЗИТЬ(ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Документ.ОтгрузкаТоваровСХранения).ХозяйственнаяОперация =
		|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
	...
	Если ПравоДоступаЗаказДавальца2_5 Тогда
		...
		|	КОНЕЦ                                                            КАК СкладОтгрузки,                
		|	НЕ ЗаказКлиента.СкладВШапкеЗаказа                                КАК ЗапрещеноВыбиратьГруппуСкладов,
		|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5) КАК ХозяйственнаяОперация,
		|	ЗаказКлиентаТовары.Количество                                    КАК КОформлениюОстаток,
		|	ЗаказКлиента.Валюта                                              КАК ВалютаВзаиморасчетов,
		|	ЗаказКлиента.НалогообложениеНДС                                  КАК НалогообложениеНДС,
		|	ЗаказКлиента.ЦенаВключаетНДС                                     КАК ЦенаВключаетНДС,
		...
		|	ЗаказКлиента.СпособДоставки                                      КАК СпособДоставки,
		|	ВЫБОР
		|		КОГДА ЗаказКлиента.СпособДоставки В (
		|									ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчика),
		|									ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчикаПоАдресу))
		|			ТОГДА ЗаказКлиента.ПеревозчикПартнер
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)
		...
		|	ВтЗаказыКлиентов КАК ЗаказКлиента
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца2_5.Продукция КАК ЗаказКлиентаТовары
		|	ПО ЗаказКлиента.Ссылка = ЗаказКлиентаТовары.Ссылка
		|	И (ЗаказКлиента.СкладВШапкеЗаказа ИЛИ ЗаказКлиента.Склад = ЗаказКлиентаТовары.Склад)
		|
		|
...
Функция РСК_ПолучитьРеквизитыРаспоряжений(МассивРаспоряжений)
	...
	//++ НЕ УТКА
	ЕстьДоступКЗаказамДавальцев		= ПравоДоступа("Чтение", Метаданные.Документы.ЗаказДавальца2_5);
	//-- НЕ УТКА
	...
	Если Не ЕстьДоступКЗаказамКлиентов
		...
		//++ НЕ УТКА
		И Не ЕстьДоступКЗаказамДавальцев
		//-- НЕ УТКА
	...
	//++ НЕ УТКА
	Если ЕстьДоступКЗаказамДавальцев Тогда
		
		ТекстЗапросаЗаказыДавальцев = 
		"ВЫБРАТЬ
		|	ЗаказДавальца2_5.Контрагент                            КАК Контрагент,
		|	ЗаказДавальца2_5.Организация                           КАК Организация,
		...
		|	ЗаказДавальца2_5.Грузоотправитель                      КАК Грузоотправитель,
		|	ЗаказДавальца2_5.Грузополучатель                       КАК Грузополучатель,
		|	ЗаказДавальца2_5.БанковскийСчетГрузоотправителя        КАК БанковскийСчетГрузоотправителя,
		|	ЗаказДавальца2_5.БанковскийСчетГрузополучателя         КАК БанковскийСчетГрузополучателя,
		...
		|	ЗаказДавальца2_5.АдресДоставкиПеревозчикаЗначенияПолей КАК АдресДоставкиПеревозчикаЗначенияПолей,
		...
		|	Документ.ЗаказДавальца2_5 КАК ЗаказДавальца2_5
		|ГДЕ
		|	ЗаказДавальца2_5.Ссылка В(&МассивРаспоряжений)";
		
		ПолеВТИсточникиДанных = "";
		Если ТекстыИсточниковДанных.Количество() = 0 Тогда
			ПолеВТИсточникиДанных = "ПОМЕСТИТЬ ВТИсточникиДанных";
		КонецЕсли;
		
		ТекстЗапросаЗаказыДавальцев =
			СтрЗаменить(ТекстЗапросаЗаказыДавальцев, ",&ВТИсточникиДанных", ПолеВТИсточникиДанных);
		
		ТекстыИсточниковДанных.Добавить(ТекстЗапросаЗаказыДавальцев);
...
&После("ВыполнитьКонтрольЗаказаПослеПроведения")
Процедура РСК_ВыполнитьКонтрольЗаказаПослеПроведения(КонтролируемыйЗаказ, Отказ)
	...
	//++ РС Консалт: Трофимов Евгений 25.01.2023 Тикет 23331
	//e1cib/data/Документ.Задание?ref=a98333d1ebaaf75b4b48c37d90a42618
	//Запрос = Новый Запрос(
	//"ВЫБРАТЬ
	//|	ЗаказКлиента.Валюта КАК Валюта,
	//|	ЗаказКлиента.СуммаДокумента - СостоянияЗаказовКлиентов.СуммаОплаты КАК КОплатеОстаток
	//|ИЗ
	//|	РегистрСведений.СостоянияЗаказовКлиентов КАК СостоянияЗаказовКлиентов
	//|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента КАК ЗаказКлиента
	//|		ПО (&КонтролироватьРасчеты)
	//|			И (ЗаказКлиента.Ссылка = &Ссылка)
	//|			И НЕ (ЗаказКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам))
	//|			И СостоянияЗаказовКлиентов.Заказ = ЗаказКлиента.Ссылка
	...
	ВыборкаРасчеты = Запрос.Выполнить().Выбрать(); 
	...
	Если ВыборкаРасчеты.Следующий() Тогда 
		Если ТипЗнч(КонтролируемыйЗаказ) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
			...
			|en = 'Settlements against the ""%Заказ%"" document are not completed.
			|To close the order, payment is required: %СуммаКОплате% %Валюта%.
			|You can close the order only with fully paid/canceled lines'")
		КонецЕсли;
