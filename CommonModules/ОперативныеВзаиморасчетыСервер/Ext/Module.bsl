...
&ИзменениеИКонтроль("ЗаполнитьОперативныеВзаиморасчеты")
Процедура РСК_ЗаполнитьОперативныеВзаиморасчеты(ОсновныеПараметры)
	...
	|	ЕСТЬNULL(КурсыВалютПоДоговору.КурсЗнаменатель, 1) КАК КратностьРегл,
	...
	Если РеквизитыОбъектаРасчетов.Количество() = 1 Тогда
		...
		Если ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.УстановленныйВДоговоре Тогда
			КратностьРегл = РеквизитыОбъектаРасчетов[0].КратностьРегл;
			Если ВалютаУправленческогоУчета = ВалютаРегламентированногоУчета Тогда
				КратностьУпр = КратностьРегл;
			КонецЕсли;
		КонецЕсли;
	Иначе
	...
	Запрос.УстановитьПараметр("ФиксированныйКурсРегл",      ФиксированныйКурсРегл);
	Запрос.УстановитьПараметр("ФиксированныйКурсУпр",       ФиксированныйКурсУпр);
	Запрос.УстановитьПараметр("ФиксированныйКурсНакладной", ФиксированныйКурсНакладной);
				...
				Если СтрокаОтгрузкиПоставки.Сумма > 0 И СтрокаОплаты.Сумма > 0 Тогда
					...
					Если (СтрокаОтгрузкиПоставки.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВзаимозачетЗадолженности
						...
						ПредоплатаРеглПредв = СуммаСписания(СтрокаОтгрузкиПоставки.КурсРегл, СтрокаОтгрузкиПоставки.СуммаРегл,
							 СтрокаОтгрузкиПоставки.Сумма, СуммаСписания);
					//Остальные авансы по курсу аванса.
					Иначе
						ПредоплатаРеглПредв = СуммаСписания(0, СтрокаОплаты.СуммаРегл, СтрокаОплаты.Сумма, СуммаСписания);
					КонецЕсли;
					...
					Если (СтрокаОтгрузкиПоставки.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВзаимозачетЗадолженности
						...
						ПредоплатаУпрПредв = СуммаСписания(СтрокаОтгрузкиПоставки.КурсУпр, СтрокаОтгрузкиПоставки.СуммаУпр,
							СтрокаОтгрузкиПоставки.Сумма, СуммаСписания);
					Иначе
						ПредоплатаУпрПредв = СуммаСписания(0, СтрокаОплаты.СуммаУпр, СтрокаОплаты.Сумма, СуммаСписания);
					КонецЕсли;
			...
			Иначе
				...
				Если НЕ СтрокаОплаты.ЭтоКорректировка Тогда
					...
					Если СтрокаОтгрузкиПоставки.Сумма > 0 И СтрокаОплаты.Сумма > 0 Тогда
						...
						// Списываем долг по курсу накладной для варианта курса договора, фиксированного на дату отгрузки
						// или по курсу аванса для остальных вариантов курса договора и для ручного курса
						ДолгРегл = СуммаСписания(0, СтрокаОплаты.СуммаРегл, СтрокаОплаты.Сумма, СуммаСписания);
						ДолгУпр = СуммаСписания(0, СтрокаОплаты.СуммаУпр, СтрокаОплаты.Сумма, СуммаСписания);
						...
						// Долг для фиксированного курса нужно переоценить, если произошла частичная оплата по другому курсу
						Если НужнаПереоценка
							И ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.ФиксированныйНаДатуОтгрузки Тогда
							ПереоценитьДолг(ГлобальныеПеременные, ТаблицаРасчетовПоСрокам, РасчетыОтгрузкиПоставки, СтрокаОтгрузкиПоставки.Регистратор, СтрокаОплаты);
						КонецЕсли;
					КонецЕсли;
				...
				Иначе
					...
					ОстаткиСвязанногоДокумента = РасчетыОтгрузкиПоставки.НайтиСтроки(Новый Структура("ЕстьОстаток, РасчетныйДокумент", ИСТИНА, СтрокаОплаты.СвязанныйДокумент));
					...
					Для Каждого СтрокаОстаткаСвязанногоДокумента Из ОстаткиСвязанногоДокумента Цикл
						...
						Иначе //Если это корректировка и накладная еще не оплачена полностью, то расчетным документом должна являться накладная.
							...
							Если СтрокаОплаты.Сумма > 0 И СтрокаОстаткаСвязанногоДокумента.Сумма > 0 Тогда
								...
								// Долг для фиксированного курса нужно переоценить, если произошла корректировка по другому курсу
								Если НужнаПереоценка
									И ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.ФиксированныйНаДатуОтгрузки Тогда
									ПереоценитьДолг(ГлобальныеПеременные, ТаблицаРасчетовПоСрокам, РасчетыОтгрузкиПоставки, СтрокаОтгрузкиПоставки.Регистратор, СтрокаОплаты);
								КонецЕсли;
	...
	Если ГлобальныеПеременные.РассчитыватьПланы И ПланыОплат.Количество() > 0 Тогда
		...
		ПланыОплат.Сортировать("ПорядокОперации ВОЗР, ПорядокЗачетаПоДатеПлатежа ВОЗР, ЕстьДокументПлан УБЫВ, ВариантОплаты");
	...
	Если НЕ ОсновныеПараметры.НачальноеЗаполнение И ОсновныеПараметры.ЗаписыватьИзменения Тогда
		...
		Если ОсновныеПараметры.Свойство("ДополнительныеСвойстваПроведения") И ОсновныеПараметры.ДополнительныеСвойстваПроведения <> Неопределено Тогда
			...
			#Вставка
			//++РС Консалт: Минаков А.С. Тикет #308 ОРТ
			|ГДЕ НЕ ВложенныйЗапрос.ДокументРегистратор ССЫЛКА Документ.РасчетКурсовыхРазниц
			//++РС Консалт: Минаков А.С. Тикет #308 ОРТ
			#КонецВставки
			|СГРУППИРОВАТЬ ПО
			|	Период,
			...
			Иначе
				...
				ТаблицаИзменений = Запрос.Выполнить().Выгрузить();
				#Вставка
				//++ РС Консалт: Трофимов Евгений 26.01.2023 Задача 23250
				//e1cib/data/Документ.Задание?ref=946a2b23e0d9052f4b2662deccc100ca
				//Сергей Макаров (Skype): изменение заказа поставщику не ломает закрытие и взаиморасчеты
				//надо дать возможность менять
				Если ТипЗнч(НеотражаемыйДокумент) <> Тип("ДокументСсылка.ЗаказПоставщику") Тогда
				//-- КонецЗадачи 23250				
				#КонецВставки
