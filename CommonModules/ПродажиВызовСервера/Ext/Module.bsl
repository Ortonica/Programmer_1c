...
Функция РСК_СформироватьДанныеЗаполненияРеализации(МассивСсылок, ИмяДокумента, РеквизитыШапки)
	...
	//++ РС Консалт: Минаков А.С. Задача 22756
		...
		//++ НЕ УТКА
		|		И НЕ &ЭтоОтгрузкаТоваровСХранения
		//-- НЕ УТКА
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказДавальца2_5) Тогда
		...
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ЗаказДавальца2_5.Ссылка                                          КАК ЗаказКлиента,
		...
		|	ВЫБОР
		|		КОГДА ЗаказДавальца2_5.Склад.ЭтоГруппа
		|			И ЗаказДавальца2_5.Склад.ВыборГруппы = ЗНАЧЕНИЕ(Перечисление.ВыборГруппыСкладов.РазрешитьВЗаказах)
		|				ТОГДА ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|		ИНАЧЕ ЗаказДавальца2_5.Склад
		|	КОНЕЦ                                                            КАК СкладОтгрузки,
		|	ЗаказДавальца2_5.СпособДоставки                                  КАК СпособДоставки,
		|	ВЫБОР
		|		КОГДА ЗаказДавальца2_5.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчика)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		...
		|	ВЫБОР
		|		КОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5) В (&ДопустимыеОперации)
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ                                                            КАК ЕстьНедопустимаяОперация
		|ПОМЕСТИТЬ ВтЗаказыКлиента
		|ИЗ
		...
		Если ЗначениеЗаполнено(ТекстЗаказы) Тогда
			
			ТекстыЗапроса = Новый Массив;
			ТекстыЗапроса.Добавить(ТекстЗаказы);
			ТекстыЗапроса.Добавить(СтрЗаменить(ТекстЗапроса, "ПОМЕСТИТЬ ВтЗаказыКлиента", ""));
			
			ТекстЗапроса = СтрСоединить(ТекстыЗапроса, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
			
		КонецЕсли;
			
		ТекстЗаказы = ТекстЗапроса;
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказДавальца2_5) Тогда
		...
		|	НЕ ЗаказДавальца2_5.Статус В (&МассивСтатусовЗаказДавальца) КАК ЕстьОшибкиСтатус
		|ИЗ
		|	Документ.ЗаказДавальца2_5 КАК ЗаказДавальца2_5
		|ГДЕ
		|	ЗаказДавальца2_5.Ссылка В (&МассивСсылок)
		|	И ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5) В (&ДопустимыеОперации)
		|	И (НЕ ЗаказДавальца2_5.Проведен ИЛИ НЕ ЗаказДавальца2_5.Статус В (&МассивСтатусовЗаказДавальца))";
		
		Если ЗначениеЗаполнено(ТекстДанныеДокументов) Тогда
			
			ТекстыЗапроса = Новый Массив;
			ТекстыЗапроса.Добавить(ТекстДанныеДокументов);
		...
		ТекстДанныеДокументов = ТекстЗапроса;
	...
	//++ НЕ УТКА
	ЭтоОтгрузкаТоваровСХранения = ?(ИмяДокумента = Метаданные.Документы.ОтгрузкаТоваровСХранения.Имя, Истина, Ложь);
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	ИначеЕсли ЭтоОтгрузкаТоваровСХранения Тогда
		ИмяДокументаРТУ = НСтр("ru = 'передачи давальцу';
								|en = 'transfers to material provider'");
	//-- НЕ УТКА
	Иначе
	...
	//++ НЕ УТКА
	Запрос.УстановитьПараметр("ЭтоОтгрузкаТоваровСХранения", ЭтоОтгрузкаТоваровСХранения);
	//-- НЕ УТКА
	...
	Запрос.УстановитьПараметр(
		"МассивСтатусовЗаказДавальца",
		Документы.ЗаказДавальца2_5.ДопустимыеСтатусыВводаНаОсновании(ИмяДокумента));
	//-- НЕ УТКА
	...
	Иначе
		...
		Если Не РезультатЗапроса[4].Пустой() > 0 Тогда
			...
			Пока ВыборкаЗаказы.Следующий() Цикл
				...
				//++ НЕ УТКА
				Если ТипЗнч(ВыборкаЗаказы.ЗаказКлиента) = Тип("ДокументСсылка.ЗаказДавальца2_5") Тогда
					
					Документы.ЗаказДавальца2_5.ПроверитьВозможностьВводаНаОсновании(
				...
				//-- НЕ УТКА
					...
					Документы[ВыборкаЗаказы.ЗаказКлиента.Метаданные().Имя].ПроверитьВозможностьВводаНаОсновании(
						ВыборкаЗаказы.ЗаказКлиента,
						ВыборкаЗаказы.Статус,
						ВыборкаЗаказы.ЕстьОшибкиПроведен);
