...
Процедура РСК_ЗаполнитьМенеджера(ДокументОбъект, Перезаполнить, ПоляПоиска)
	...
	ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) 			= Тип("ДокументСсылка.ВозвратТоваровОтКлиента")
		...
		//КонецЕсли;
		//Если ЗначениеЗаполнено(ДокументОбъект.ЗаказКлиента) Тогда //здесь возникала ошибка так как документе ВозвратТоваровОтКлиента нет реквизита ЗаказКлиента. Будем брать из реализации
		...
		//КонецЕсли;
		Если ЗначениеЗаполнено(ДокументОбъект.ДокументРеализации) Тогда
			СтруктураМенеджера = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				ДокументОбъект.ДокументРеализации,
				"ЗаказКлиента.Менеджер,ЗаказКлиента.Менеджер.Подразделение",
				Истина
			);
			ДокументОбъект.Менеджер = СтруктураМенеджера.ЗаказКлиентаМенеджер;
			ДокументОбъект.Подразделение = СтруктураМенеджера.ЗаказКлиентаМенеджерПодразделение;
		КонецЕсли;
	КонецЕсли;   
	//-- Конец
	#КонецВставки
