...
Процедура РСК_СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ)
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияРасчетыСКлиентамиИзменениеКонтрольСрока") Тогда
		...
		Иначе
			...
			//СвойствоПДЗ = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Разрешить проведение с просроченной дебиторской задолженностью");
			//Если УправлениеСвойствами.ПроверитьСвойствоУОбъекта(Документ.Ссылка, СвойствоПДЗ) Тогда
			//	ИгнорироватьПДЗ = УправлениеСвойствами.ЗначениеСвойства(Документ.Ссылка,СвойствоПДЗ);
			//КонецЕсли;
			//РазрешитьПроведение = ?(ИгнорироватьПДЗ = Неопределено, Ложь, ИгнорироватьПДЗ);
