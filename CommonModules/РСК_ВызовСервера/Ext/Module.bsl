...
Функция МаксимальноеЗначениеВМассиве(МассивЗначений) Экспорт
	...
	Решение = МассивЗначений[0];
	Для Каждого Значение Из МассивЗначений Цикл
		Решение = Макс(Решение, Значение);
	КонецЦикла;
	
	Возврат Решение;

КонецФункции // МаксимальноеЗначениеВМассиве()

Функция МинимальноеЗначениеВМассиве(МассивЗначений) Экспорт
...
Процедура ЗапускВыполненияФоновогоИсключенияРасходногоОрдера(ЗаданиеНаПеревозку) Экспорт
	...
	//++ РС Консалт: Трофимов Евгений 18.02.2023 Задача 23698
	...
	//	
	//Стало:
	СтруктураПараметров = Новый Структура("ЗаданиеНаПеревозку", ЗаданиеНаПеревозку);
    ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор());
    ПараметрыВыполнения.ОжидатьЗавершение = 0; // запускать сразу
    ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр(
		СтрШаблон("ru = 'Обработка РО для %1'", ЗаданиеНаПеревозку)
	);
    ПараметрыВыполнения.ЗапуститьНеВФоне = Ложь;
     
    РезультатВыполнения = ДлительныеОперации.ВыполнитьВФоне(
        "РСК_ВызовСервера.ВыполнитьИсключениеРасходногоОрдера",
        СтруктураПараметров, 
        ПараметрыВыполнения
    );
...
Процедура ВыполнитьИсключениеРасходногоОрдера(Параметры, АдресХранилища = "") Экспорт
	...
	//e1cib/data/Документ.Задание?ref=a1c07de7eccf0a5942be9c5a46bb066d
	ТаблицыСверкиСтатусовРО = Неопределено;
	ТекстОшибок = "";
	Если РегистрыСведений.реа_ПредопределенныеЗначения.Значение("Функционал задачи № 243812", Истина) Тогда
		Если СкладыСервер.ИспользоватьОрдернуюСхемуПриОтгрузке(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаданиеНаПеревозку,"Склад"), ДатаОтгрузки) Тогда

			ТаблицыСверкиСтатусовРО = Документы.ЗаданиеНаПеревозку.ПолучитьТаблицыСверкиСтатусовРО(Новый Структура("Ссылка", ЗаданиеНаПеревозку));
			Если ТаблицыСверкиСтатусовРО.РаспоряженияБезРО.Количество() > 0 Тогда
				Статус = ТаблицыСверкиСтатусовРО.РаспоряженияБезРО[0].СтатусМЛ;
				ТекстОшибок = ТекстОшибок + ?(ПустаяСтрока(ТекстОшибок), "", Символы.ПС) + "Нельзя устанавливать статус "+Статус+" т.к. имеются распоряжения без расходных ордеров:";
				Для Каждого Стр Из ТаблицыСверкиСтатусовРО.РаспоряженияБезРО Цикл
					ТекстОшибок = ТекстОшибок + ?(ПустаяСтрока(ТекстОшибок), "", Символы.ПС) + ""+Стр.Распоряжение;
				КонецЦикла;
			КонецЕсли;
			Если ТаблицыСверкиСтатусовРО.НедопустимыеСтатусыРО.Количество() > 0 Тогда
				ТекстОшибок = ТекстОшибок + ?(ПустаяСтрока(ТекстОшибок), "", Символы.ПС);
				Статус = ТаблицыСверкиСтатусовРО.НедопустимыеСтатусыРО[0].СтатусМЛ;
				ТекстОшибок = ТекстОшибок + ?(ПустаяСтрока(ТекстОшибок), "", Символы.ПС) + "Нельзя устанавливать статус "+Статус+" т.к. имеются расходные ордера со статусами ниже чем «К отгрузке»:";
				Для Каждого Стр Из ТаблицыСверкиСтатусовРО.НедопустимыеСтатусыРО Цикл
					ТекстОшибок = ТекстОшибок + ?(ПустаяСтрока(ТекстОшибок), "", Символы.ПС) + Стр.РО+" имеет статус "+Стр.СтатусРО;
				КонецЦикла;
			КонецЕсли;
			
			Если ПустаяСтрока(ТекстОшибок) Тогда
			//	СинхронизироватьСтатусыРО_КПогрузкеОтправлено(ЗаданиеНаПеревозку, ТаблицыСверкиСтатусовРО.КСменеСтатуса);
			//	ЗаписьЖурналаРегистрации("Синхронизация статусов РО", УровеньЖурналаРегистрации.Предупреждение, ЗаданиеНаПеревозку.Метаданные(), ЗаданиеНаПеревозку, "Статусы РО успешно синхронизированы");
			Иначе
				ЗаписьЖурналаРегистрации("Синхронизация статусов РО", УровеньЖурналаРегистрации.Ошибка, ЗаданиеНаПеревозку.Метаданные(), ЗаданиеНаПеревозку, ТекстОшибок);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	//-- КонецЗадачи 23698	
	...
	Для Каждого ДанныеСтроки Из ЗаданиеНаПеревозку.Распоряжения Цикл
		...
		//e1cib/data/Документ.Задание?ref=a1c07de7eccf0a5942be9c5a46bb066d
		Если ТаблицыСверкиСтатусовРО <> Неопределено Тогда
			Если ПустаяСтрока(ТекстОшибок) Тогда
				ПараметрыОбновленияОрдера.Вставить("КСменеСтатуса", ТаблицыСверкиСтатусовРО.КСменеСтатуса);
				ПараметрыОбновленияОрдера.Вставить("ОтветственныйЗаОтгрузку", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаданиеНаПеревозку, "РСК_ОтветственныйЗаОтгрузку"));
			КонецЕсли;
		КонецЕсли;
		//-- КонецЗадачи 23698		
	...
	//++ РС Консалт: Трофимов Евгений 17.02.2023 Задача 23698
	//e1cib/data/Документ.Задание?ref=a1c07de7eccf0a5942be9c5a46bb066d
	Если ТаблицыСверкиСтатусовРО <> Неопределено И ПустаяСтрока(ТекстОшибок) Тогда
		СинхронизироватьСтатусыРО_КПогрузкеОтправлено(ЗаданиеНаПеревозку, ТаблицыСверкиСтатусовРО.КСменеСтатуса);
	КонецЕсли;
	ЗаписьЖурналаРегистрации("Синхронизация статусов РО", УровеньЖурналаРегистрации.Предупреждение, ЗаданиеНаПеревозку.Метаданные(), ЗаданиеНаПеревозку, "Статусы РО успешно синхронизированы");
    //-- КонецЗадачи 23698
...
Процедура ВыполнитьПроверкуИзменениеОрдера(ПараметрыОбновленияОрдера, Отказ) Экспорт
	...
	//++ РС Консалт: Трофимов Евгений 20.02.2023 Задача 23698
	//e1cib/data/Документ.Задание?ref=a1c07de7eccf0a5942be9c5a46bb066d
	Если ПараметрыОбновленияОрдера.Свойство("КСменеСтатуса") Тогда
		СтрокаСОрдером = ПараметрыОбновленияОрдера.КСменеСтатуса.Найти(Ордер, "РО");
		Если СтрокаСОрдером <> Неопределено Тогда
			Если СтрокаСОрдером.СтатусРО = Перечисления.СтатусыРасходныхОрдеров.КОтгрузке 
				ИЛИ СтрокаСОрдером.СтатусРО = Перечисления.СтатусыРасходныхОрдеров.Отгружен Тогда
				ОрдерОбъект.Статус = СтрокаСОрдером.ТребуемыйСтатусРО;
				БылоИзменениеСтатуса = Истина;
			КонецЕсли;
			Если ОрдерОбъект.Статус = Перечисления.СтатусыРасходныхОрдеров.Отгружен И ЗначениеЗаполнено(ПараметрыОбновленияОрдера.ОтветственныйЗаОтгрузку) Тогда
				НайденныеСтроки = ОрдерОбъект.Ответственные.НайтиСтроки(Новый Структура("Пользователь", ПараметрыОбновленияОрдера.ОтветственныйЗаОтгрузку));
				Если НайденныеСтроки.Количество() = 0 Тогда
					НС = ОрдерОбъект.Ответственные.Добавить();
					НС.Пользователь = ПараметрыОбновленияОрдера.ОтветственныйЗаОтгрузку;
					БылоИзменениеСтатуса = Истина;
				КонецЕсли;
			КонецЕсли;
			ПараметрыОбновленияОрдера.КСменеСтатуса.Удалить(СтрокаСОрдером);
		КонецЕсли;
	КонецЕсли;
	//-- КонецЗадачи 23698		
	...
	Если БылоИзменениеЗадание Или БылоИзменениеСтатуса Или БылоЗаполнение Тогда
		Попытка
			Если ОрдерОбъект.ПроверитьЗаполнение() Тогда
				...
				Если БылоИзменениеЗадание И БылоИзменениеСтатуса Тогда
					ТекстСообщения = СтрШаблон("%1: Включен в %2 и переведен в статус «%3»",Ордер,ЗаданиеНаПеревозку,ОрдерОбъект.Статус);
					//ТекстСообщения = "" + Ордер + ": Включен в " + ЗаданиеНаПеревозку + " и переведен в статус ""К отгрузке"""
				ИначеЕсли БылоИзменениеЗадание Тогда
					ТекстСообщения = СтрШаблон("%1: Включен в %2",Ордер,ЗаданиеНаПеревозку);
					//ТекстСообщения = "" + Ордер + ": Включен в " + ЗаданиеНаПеревозку
				Иначе
					ТекстСообщения = СтрШаблон("%1 переведен в статус «%2»",Ордер,ОрдерОбъект.Статус);
			...
			Иначе
				Если БылоИзменениеЗадание И БылоИзменениеСтатуса Тогда
					ТекстСообщения = СтрШаблон(
						"%1: Заполнен некорректно и не будет включен в %2 и не переведен в статус «%3»",
						Ордер,ЗаданиеНаПеревозку,ОрдерОбъект.Статус
					);
					//ТекстСообщения = "" + Ордер + ": Заполнен некорректно и не будет включен в " + ЗаданиеНаПеревозку + " и не переведен в статус ""К отгрузке"""
				ИначеЕсли БылоИзменениеЗадание Тогда
					ТекстСообщения = СтрШаблон("%1: Заполнен некорректно и не будет включен в %2",Ордер,ЗаданиеНаПеревозку);
					//ТекстСообщения = "" + Ордер + ": Заполнен некорректно и не будет включен в " + ЗаданиеНаПеревозку
				Иначе 
					ТекстСообщения = СтрШаблон(
						"%1: Заполнен некорректно и не переведен в статус «%2»",
						Ордер,ОрдерОбъект.Статус
					);
		...
		Исключение
			...
			Если БылоИзменениеЗадание И БылоИзменениеСтатуса Тогда
				ПараметрСообщения = "включении в " + ЗаданиеНаПеревозку + " и переводе в статус "+ОрдерОбъект.Статус
			ИначеЕсли БылоИзменениеЗадание Тогда
				ПараметрСообщения = "включении в " + ЗаданиеНаПеревозку
			Иначе 
				ПараметрСообщения = "переводе в статус " +ОрдерОбъект.Статус
			КонецЕсли;
...
// Выполняет смену статуса в РО, где имеются отличие от статуса МЛ
//РС Консалт: Трофимов Евгений 18.02.2023 Задача 23698
//e1cib/data/Документ.Задание?ref=a1c07de7eccf0a5942be9c5a46bb066d
//
// Параметры:
//	ЗаданиеНаПеревозку - ДокументСсылка.ЗаданиеНаПеревозку - Маршрутный лист
//  КСменеСтатуса	   - ТаблицаЗначений - Таблица, формируемая в методе Документы.ЗаданиеНаПеревозку.ПолучитьТаблицыСверкиСтатусовРО()
//		* РО 				- ДокументСсылка.РасходныйОрдерНаТовары - Документ РО
//		* ТребуемыйСтатусРО - ПеречислениеСсылка.СтатусыРасходныхОрдеров - Требуемый статус
//		* СтатусРО			- ПеречислениеСсылка.СтатусыРасходныхОрдеров - Текущий статус
//
Процедура СинхронизироватьСтатусыРО_КПогрузкеОтправлено(ЗаданиеНаПеревозку, КСменеСтатуса)
	Если КСменеСтатуса.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	ЗаписьЖурналаРегистрации("Синхронизация статусов РО", УровеньЖурналаРегистрации.Предупреждение, ЗаданиеНаПеревозку.Метаданные(), ЗаданиеНаПеревозку, "Есть неучтённые РО! Обработка их пойдёт ниже.");
	ОтветственныйЗаОтгрузку = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаданиеНаПеревозку, "РСК_ОтветственныйЗаОтгрузку");
	Для Каждого Стр Из КСменеСтатуса Цикл
		оРО = Стр.РО.ПолучитьОбъект();
		оРО.Заблокировать();
		
		//++ РС Консалт: Трофимов Евгений 05.04.2023 Тикет 24985
		//e1cib/data/Документ.Задание?ref=9a3c01eb245c24dd4ef5d9b0182f94ff
		//Наталья Калинина: при сканировании МЛ-есть вероятность, что там дата не изменяется на текущую, вот это надо доработать ТОЧНО
		Если оРО.Статус <> Стр.ТребуемыйСтатусРО И Стр.ТребуемыйСтатусРО = Перечисления.СтатусыРасходныхОрдеров.Отгружен Тогда
			оРО.ДатаОтгрузки = ТекущаяДатаСеанса();
		КонецЕсли;
		//-- КонецТикета 24985		
		
		Если Стр.СтатусРО = Перечисления.СтатусыРасходныхОрдеров.КОтгрузке 
			ИЛИ Стр.СтатусРО = Перечисления.СтатусыРасходныхОрдеров.Отгружен Тогда
			оРО.Статус = Стр.ТребуемыйСтатусРО;
		КонецЕсли;
		
		Если оРО.Статус = Перечисления.СтатусыРасходныхОрдеров.Отгружен И ЗначениеЗаполнено(ОтветственныйЗаОтгрузку) Тогда
			НайденныеСтроки = оРО.Ответственные.НайтиСтроки(Новый Структура("Пользователь", ОтветственныйЗаОтгрузку));
			Если НайденныеСтроки.Количество() = 0 Тогда
				НС = оРО.Ответственные.Добавить();
				НС.Пользователь = ОтветственныйЗаОтгрузку;
			КонецЕсли;
		КонецЕсли;
		
		Попытка
			оРО.Записать(РежимЗаписиДокумента[?(оРО.Проведен, "Проведение", "Запись")]);
			ЗаписьЖурналаРегистрации("Синхронизация статусов РО", УровеньЖурналаРегистрации.Информация, оРО.Метаданные(), оРО.Ссылка, "Статус РО успешно изменён");
		Исключение
			ЗаписьЖурналаРегистрации("Синхронизация статусов РО", УровеньЖурналаРегистрации.Ошибка, оРО.Метаданные(), оРО.Ссылка, ОписаниеОшибки());
		КонецПопытки;
	КонецЦикла;
...
Процедура Пауза(ПериодОжиданияВСекундах = 10) Экспорт

	Параметры = Новый Массив;
	Параметры.Добавить(ПериодОжиданияВСекундах);
...
//++ Конарев И.И. Пересчет ВГХ по техническим характеристикам и упаковкам номенклатуры
Функция РассчитатьВесогабаритныеХарактеристикиДокумента(Товары) Экспорт
	
	РассчитыватьВГХКОтгрузке = Товары.Колонки.Найти("ВариантОбеспечения") <> Неопределено;
	...
	|	Товары.Серия КАК Серия,
	|	""Вес брутто"" КАК Вес,
	|	""Высота упаковки"" КАК Высота,
	|	""Ширина упаковки"" КАК Ширина,
	|	""Длина упаковки"" КАК Длина,
	|	Товары.Упаковка КАК Упаковка,
	|	Товары.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить) КАК КОтгрузке,
	|	Товары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка) КАК ПустаяУпаковка,
	...
	|	ВЫБОР
	|		КОГДА САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование = ""Вес брутто""
	|			ТОГДА САВ_ХарактеристикиНоменклатуры.ЗначениеПоУмолчанию
	|	КОНЕЦ КАК Вес,
	|	ВЫБОР
	|		КОГДА САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование = ""Длина упаковки""
	|			ТОГДА САВ_ХарактеристикиНоменклатуры.ЗначениеПоУмолчанию
	|	КОНЕЦ КАК Длина,
	|	ВЫБОР
	|		КОГДА САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование = ""Высота упаковки""
	|			ТОГДА САВ_ХарактеристикиНоменклатуры.ЗначениеПоУмолчанию
	|	КОНЕЦ КАК Высота,
	|	ВЫБОР
	|		КОГДА САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование = ""Ширина упаковки""
	|			ТОГДА САВ_ХарактеристикиНоменклатуры.ЗначениеПоУмолчанию
	|	КОНЕЦ КАК Ширина
	|ПОМЕСТИТЬ ВГХ_Номенклатура
	|ИЗ
	|	ВТ_Товары КАК ВТ_Товары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.САВ_ХарактеристикиНоменклатуры КАК САВ_ХарактеристикиНоменклатуры
	|		ПО ВТ_Товары.Номенклатура = САВ_ХарактеристикиНоменклатуры.ВладелецПараметра
	|			И (ВТ_Товары.Вес = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ВТ_Товары.Высота = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ВТ_Товары.Ширина = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ВТ_Товары.Длина = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|				ТОГДА САВ_ХарактеристикиНоменклатуры.ЗначениеПоУмолчанию
	|		КОНЕЦ) КАК Ширина
	|ПОМЕСТИТЬ ВГХ_Характеристики
	|ИЗ
	|	ВТ_Товары КАК ВТ_Товары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.САВ_ХарактеристикиНоменклатуры КАК САВ_ХарактеристикиНоменклатуры
	|		ПО ВТ_Товары.Характеристика = САВ_ХарактеристикиНоменклатуры.ВладелецПараметра
	|			И (ВТ_Товары.Вес = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ВТ_Товары.Высота = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ВТ_Товары.Ширина = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ВТ_Товары.Длина = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование)
	...
	|	ВТ_Товары КАК ВТ_Товары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.САВ_ХарактеристикиНоменклатуры КАК САВ_ХарактеристикиНоменклатуры
	|		ПО ВТ_Товары.Серия = САВ_ХарактеристикиНоменклатуры.ВладелецПараметра
	|			И (ВТ_Товары.Вес = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ВТ_Товары.Высота = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ВТ_Товары.Ширина = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ВТ_Товары.Длина = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование)
	...
	|	ВТ_Товары.Серия КАК Серия,
	|	ВЫБОР
	|		КОГДА ВГХ_Серии.Вес ЕСТЬ NULL
	|			ТОГДА ВЫБОР
	|					КОГДА ВГХ_Характеристики.Вес ЕСТЬ NULL
	|						ТОГДА ВЫБОР
	|								КОГДА ВГХ_Номенклатура.Вес ЕСТЬ NULL
	|									ТОГДА ""Упаковка""
	|								ИНАЧЕ ВГХ_Номенклатура.Вес
	|							КОНЕЦ
	|					ИНАЧЕ ВГХ_Характеристики.Вес
	|				КОНЕЦ
	|		ИНАЧЕ ВГХ_Серии.Вес
	|	КОНЕЦ КАК Вес,
	|	ВЫБОР
	|		КОГДА ВГХ_Серии.Высота ЕСТЬ NULL
	|			ТОГДА ВЫБОР
	|					КОГДА ВГХ_Характеристики.Высота ЕСТЬ NULL
	|						ТОГДА ВЫБОР
	|								КОГДА ВГХ_Номенклатура.Высота ЕСТЬ NULL
	|									ТОГДА ""Упаковка""
	|								ИНАЧЕ ВГХ_Номенклатура.Высота
	|							КОНЕЦ
	|					ИНАЧЕ ВГХ_Характеристики.Высота
	|				КОНЕЦ
	|		ИНАЧЕ ВГХ_Серии.Высота
	|	КОНЕЦ КАК Высота,
	|	ВЫБОР
	|		КОГДА ВГХ_Серии.Ширина ЕСТЬ NULL
	|			ТОГДА ВЫБОР
	|					КОГДА ВГХ_Характеристики.Ширина ЕСТЬ NULL
	|						ТОГДА ВЫБОР
	|								КОГДА ВГХ_Номенклатура.Ширина ЕСТЬ NULL
	|									ТОГДА ""Упаковка""
	|								ИНАЧЕ ВГХ_Номенклатура.Ширина
	|							КОНЕЦ
	|					ИНАЧЕ ВГХ_Характеристики.Ширина
	|				КОНЕЦ
	|		ИНАЧЕ ВГХ_Серии.Ширина
	|	КОНЕЦ КАК Ширина,
	|	ВЫБОР
	|		КОГДА ВГХ_Серии.Длина ЕСТЬ NULL
	|			ТОГДА ВЫБОР
	|					КОГДА ВГХ_Характеристики.Длина ЕСТЬ NULL
	|						ТОГДА ВЫБОР
	|								КОГДА ВГХ_Номенклатура.Длина ЕСТЬ NULL
	|									ТОГДА ""Упаковка""
	|								ИНАЧЕ ВГХ_Номенклатура.Длина
	|							КОНЕЦ
	|					ИНАЧЕ ВГХ_Характеристики.Длина
	|				КОНЕЦ
	|		ИНАЧЕ ВГХ_Серии.Длина
	|	КОНЕЦ КАК Длина,
	|	ВТ_Товары.Упаковка КАК Упаковка,
	|	ВТ_Товары.КОтгрузке КАК КОтгрузке,
	|	ВТ_Товары.ПустаяУпаковка КАК ПустаяУпаковка,
	|	ВТ_Товары.КоличествоУпаковок КАК КоличествоУпаковок
	|ПОМЕСТИТЬ ВТ_ТаблицаВГХ
	|ИЗ
	|	ВТ_Товары КАК ВТ_Товары
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВГХ_Характеристики КАК ВГХ_Характеристики
	|		ПО ВТ_Товары.Характеристика = ВГХ_Характеристики.Характеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВГХ_Номенклатура КАК ВГХ_Номенклатура
	|		ПО ВТ_Товары.Номенклатура = ВГХ_Номенклатура.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВГХ_Серии КАК ВГХ_Серии
	|		ПО ВТ_Товары.Серия = ВГХ_Серии.Серия
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТаблицаВГХ.КОтгрузке КАК КОтгрузке,
	|	ВТ_ТаблицаВГХ.Номенклатура КАК Номенклатура,
	|	ВТ_ТаблицаВГХ.Характеристика КАК Характеристика,
	|	ВТ_ТаблицаВГХ.Серия КАК Серия,
	|	СУММА(ВТ_ТаблицаВГХ.КоличествоУпаковок * ЕСТЬNULL(ВЫБОР
	|				КОГДА ВТ_ТаблицаВГХ.ПустаяУпаковка
	|					ТОГДА ШтучныйВесОбъем.Вес
	|				ИНАЧЕ УпаковкиЕдиницыИзмерения.Вес
	|			КОНЕЦ, 0)) КАК ВесУпаковки,
	|	СУММА(ВТ_ТаблицаВГХ.КоличествоУпаковок * ЕСТЬNULL(ВЫБОР
	|				КОГДА ВТ_ТаблицаВГХ.ПустаяУпаковка
	|					ТОГДА ШтучныйВесОбъем.Объем
	|				ИНАЧЕ УпаковкиЕдиницыИзмерения.Объем
	|			КОНЕЦ, 0)) КАК ОбъемУпаковки,
	|	ВТ_ТаблицаВГХ.Вес.Наименование КАК Вес,
	|	ВТ_ТаблицаВГХ.Высота.Наименование КАК Высота,
	|	ВТ_ТаблицаВГХ.Ширина.Наименование КАК Ширина,
	|	ВТ_ТаблицаВГХ.Длина.Наименование КАК Длина,
	|	СУММА(ВТ_ТаблицаВГХ.КоличествоУпаковок) КАК КоличествоУпаковок
	|ИЗ
	|	ВТ_ТаблицаВГХ КАК ВТ_ТаблицаВГХ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК УпаковкиЕдиницыИзмерения
	|		ПО ВТ_ТаблицаВГХ.Упаковка = УпаковкиЕдиницыИзмерения.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	...
	|						ВТ_ТаблицаВГХ КАК ВТ_ТаблицаВГХ
	|					ГДЕ
	|						ВТ_ТаблицаВГХ.ПустаяУпаковка)
	|			И НЕ УпаковкиЕдиницыИзмерения.ПометкаУдаления
	|			И УпаковкиЕдиницыИзмерения.Числитель = 1
	|			И УпаковкиЕдиницыИзмерения.Знаменатель = 1
	|		
	|		СГРУППИРОВАТЬ ПО
	|			УпаковкиЕдиницыИзмерения.Владелец) КАК ШтучныйВесОбъем
	|		ПО ВТ_ТаблицаВГХ.Номенклатура = ШтучныйВесОбъем.Номенклатура
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ТаблицаВГХ.Номенклатура,
	|	ВТ_ТаблицаВГХ.КОтгрузке,
	|	ВТ_ТаблицаВГХ.Вес.Наименование,
	|	ВТ_ТаблицаВГХ.Высота.Наименование,
	|	ВТ_ТаблицаВГХ.Ширина.Наименование,
	|	ВТ_ТаблицаВГХ.Длина.Наименование,
	|	ВТ_ТаблицаВГХ.Характеристика,
	|	ВТ_ТаблицаВГХ.Серия"; 
	
	Если НЕ РассчитыватьВГХКОтгрузке Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"Товары.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить)", "ЛОЖЬ");	
	КонецЕсли;
	...
	Пока Результат.Следующий() Цикл   
		...
		Если ЗначениеЗаполнено(Результат.Вес) Тогда 
			ПоказательВеса = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрРазделить(Результат.Вес," ")[1]); 
			Вес = Результат.КоличествоУпаковок * ?(ПоказательВеса <> Неопределено, ПоказательВеса, 1);		
		Иначе
			Вес = Результат.ВесУпаковки;	
		КонецЕсли; 
		
		Если ЗначениеЗаполнено(Результат.Высота) И ЗначениеЗаполнено(Результат.Ширина) И ЗначениеЗаполнено(Результат.Длина) Тогда
			ПоказательВысоты = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрРазделить(Результат.Высота," ")[1]);
			ПоказательШирины = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрРазделить(Результат.Ширина," ")[1]);
			ПоказательДлины  = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрРазделить(Результат.Длина," ")[1]); 
			
			Если ПоказательВысоты <> Неопределено И ПоказательШирины <> Неопределено И ПоказательДлины <> Неопределено Тогда
				Объем = Результат.КоличествоУпаковок * ПоказательВысоты/1000 * ПоказательШирины/1000 * ПоказательДлины/1000;
			Иначе
				Объем = Результат.КоличествоУпаковок * 1;	
			КонецЕсли; 
			
		Иначе
			Объем = Результат.ОбъемУпаковки;	
		КонецЕсли;
		
		ОбщийВес   = ОбщийВес + Вес;
		ОбщийОбъем = ОбщийОбъем + Объем; 
		Если Результат.КОтгрузке Тогда
			ВесКОтгрузке   = ВесКОтгрузке + Вес;
			ОбъемКОтгрузке = ОбъемКОтгрузке + Объем;
		КонецЕсли;
		
	КонецЦикла;
	
	Если РассчитыватьВГХКОтгрузке Тогда
		ШаблонПоказателей = "%1/%2";
		ИтоговыйВес   = СтрШаблон(ШаблонПоказателей, Окр(ОбщийВес,2), Окр(ВесКОтгрузке,2));
		ИтоговыйОбъем = СтрШаблон(ШаблонПоказателей, Окр(ОбщийОбъем,2), Окр(ОбъемКОтгрузке,2));
	Иначе
		ИтоговыйВес   = Строка(Окр(ОбщийВес,2));
		ИтоговыйОбъем = Строка(Окр(ОбщийОбъем,2));
	КонецЕсли;
	
	СтруктураВозврата = Новый Структура;
	СтруктураВозврата.Вставить("РСК_Вес", ИтоговыйВес);
	СтруктураВозврата.Вставить("РСК_Кубатура", ИтоговыйОбъем);

	Возврат СтруктураВозврата;
	
КонецФункции

Функция ПересчитатьВГХПоХарактеристикамТовара(Товары) Экспорт
	
	ТаблицаВГХ = Новый ТаблицаЗначений;
	ТаблицаВГХ.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаВГХ.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	ТаблицаВГХ.Колонки.Добавить("Серия", Новый ОписаниеТипов("СправочникСсылка.СерииНоменклатуры"));
	ТаблицаВГХ.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число"));
	ТаблицаВГХ.Колонки.Добавить("Объем", Новый ОписаниеТипов("Число"));
	ТаблицаВГХ.Колонки.Добавить("Вес", Новый ОписаниеТипов("Число")); 
	ТаблицаВГХ.Колонки.Добавить("Распоряжение");
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ВТ_ВыборкаТовары.Номенклатура КАК Номенклатура,
	|	ВТ_ВыборкаТовары.Характеристика КАК Характеристика,
	|	ВТ_ВыборкаТовары.Серия КАК Серия,
	|	ВТ_ВыборкаТовары.Количество КАК Количество,
	|	ВТ_ВыборкаТовары.Объем КАК ОбъемВыборка,
	|	""Вес брутто"" КАК Вес,
	|	""Высота упаковки"" КАК Высота,
	|	""Ширина упаковки"" КАК Ширина,
	|	""Длина упаковки"" КАК Длина,
	|	ВТ_ВыборкаТовары.Вес КАК ВесВыборка,
	|	ВТ_ВыборкаТовары.Распоряжение КАК Распоряжение
	|ПОМЕСТИТЬ ВТ_Товары
	|ИЗ
	|	&ВТ_ВыборкаТовары КАК ВТ_ВыборкаТовары
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ВТ_Товары.ОбъемВыборка КАК Объем,
	|	ВТ_Товары.Распоряжение КАК Распоряжение,
	|	ВТ_Товары.ВесВыборка КАК Вес
	|ИЗ
	|	ВТ_Товары КАК ВТ_Товары
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВГХ_Характеристики КАК ВГХ_Характеристики
	|		ПО ВТ_Товары.Характеристика = ВГХ_Характеристики.Характеристика
	...
	Запрос.УстановитьПараметр("ВТ_ВыборкаТовары", Товары);  
	
	Результат = Запрос.Выполнить().Выбрать();
	
	Пока Результат.Следующий() Цикл  
				
		СтрокаВГХ = ТаблицаВГХ.Добавить();	
		ЗаполнитьЗначенияСвойств(СтрокаВГХ,Результат);  
		
		Если ЗначениеЗаполнено(Результат.ВесВГХ) Тогда 
			ПоказательВеса = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрРазделить(Результат.ВесВГХ," ")[1]); 
			СтрокаВГХ.Вес  = Результат.Количество * ?(ПоказательВеса <> Неопределено, ПоказательВеса, 1);		
		КонецЕсли; 
		...
		Если ЗначениеЗаполнено(Результат.Высота) И ЗначениеЗаполнено(Результат.Ширина) И ЗначениеЗаполнено(Результат.Длина) Тогда
			...
			ПоказательДлины  = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрРазделить(Результат.Длина," ")[1]); 
			Если ПоказательВысоты <> Неопределено И ПоказательШирины <> Неопределено И ПоказательДлины <> Неопределено Тогда
				СтрокаВГХ.Объем = Результат.Количество * ПоказательВысоты/1000 * ПоказательШирины/1000 * ПоказательДлины/1000;
			КонецЕсли; 	
		КонецЕсли;
...
Функция РассчитатьВГХМаршрутногоЛиста(МассивОбъектов) Экспорт
	
	ТаблицаВГХ = Новый ТаблицаЗначений;
	ТаблицаВГХ.Колонки.Добавить("КоличествоШтук", Новый ОписаниеТипов("Число")); 
	ТаблицаВГХ.Колонки.Добавить("КоличествоМест", Новый ОписаниеТипов("Число"));
	ТаблицаВГХ.Колонки.Добавить("Объем",		  Новый ОписаниеТипов("Число"));
	ТаблицаВГХ.Колонки.Добавить("Вес", 			  Новый ОписаниеТипов("Число")); 
	ТаблицаВГХ.Колонки.Добавить("Распоряжение",   Метаданные.ОпределяемыеТипы.РаспоряжениеНаДоставку.Тип);
	ТаблицаВГХ.Колонки.Добавить("Ордер", 		  Новый ОписаниеТипов("ДокументСсылка.РасходныйОрдерНаТовары"));
	ТаблицаВГХ.Колонки.Добавить("Задание",		  Новый ОписаниеТипов("ДокументСсылка.ЗаданиеНаПеревозку"));
	...
	|	ТоварыРаспоряжений КАК ТоварыРаспоряжений
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.САВ_ХарактеристикиНоменклатуры КАК САВ_ХарактеристикиНоменклатуры
	|		ПО ТоварыРаспоряжений.Номенклатура = САВ_ХарактеристикиНоменклатуры.ВладелецПараметра
	|			И (ТоварыРаспоряжений.Вес = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ТоварыРаспоряжений.Высота = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ТоварыРаспоряжений.Ширина = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ТоварыРаспоряжений.Длина = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТоварыРаспоряжений.Характеристика КАК Характеристика,
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование = ""Вес брутто""
	|				ТОГДА САВ_ХарактеристикиНоменклатуры.ЗначениеПоУмолчанию
	|		КОНЕЦ) КАК Вес,
	...
	|	ТоварыРаспоряжений КАК ТоварыРаспоряжений
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.САВ_ХарактеристикиНоменклатуры КАК САВ_ХарактеристикиНоменклатуры
	|		ПО ТоварыРаспоряжений.Характеристика = САВ_ХарактеристикиНоменклатуры.ВладелецПараметра
	|			И (ТоварыРаспоряжений.Вес = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ТоварыРаспоряжений.Высота = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ТоварыРаспоряжений.Ширина = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ТоварыРаспоряжений.Длина = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТоварыРаспоряжений.Характеристика
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТоварыРаспоряжений.Серия КАК Серия,
	|	ВЫБОР
	|		КОГДА САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование = ""Вес брутто""
	|			ТОГДА САВ_ХарактеристикиНоменклатуры.ЗначениеПоУмолчанию
	|	КОНЕЦ КАК Вес,
	...
	|	ТоварыРаспоряжений КАК ТоварыРаспоряжений
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.САВ_ХарактеристикиНоменклатуры КАК САВ_ХарактеристикиНоменклатуры
	|		ПО ТоварыРаспоряжений.Серия = САВ_ХарактеристикиНоменклатуры.ВладелецПараметра
	|			И (ТоварыРаспоряжений.Вес = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ТоварыРаспоряжений.Высота = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ТоварыРаспоряжений.Ширина = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование
	|				ИЛИ ТоварыРаспоряжений.Длина = САВ_ХарактеристикиНоменклатуры.СвойствоПараметра.Наименование)
	...
	|	ТоварыРаспоряжений.Количество КАК КоличествоШтук,
	|	ТоварыРаспоряжений.Количество / ВЫБОР
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВГХ_Номенклатура КАК ВГХ_Номенклатура
	|		ПО ТоварыРаспоряжений.Номенклатура = ВГХ_Номенклатура.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВГХ_Характеристики КАК ВГХ_Характеристики
	|		ПО ТоварыРаспоряжений.Характеристика = ВГХ_Характеристики.Характеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВГХ_Серии КАК ВГХ_Серии
	|		ПО ТоварыРаспоряжений.Серия = ВГХ_Серии.Серия
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			ТОГДА 1
	|		ИНАЧЕ МаксВесОбъем.Коэффициент
	|	КОНЕЦ * ЕСТЬNULL(МаксВесОбъем.Вес, 0),
	|	ТоварыРаспоряжений.Количество / ВЫБОР
	|		КОГДА МаксВесОбъем.Коэффициент = 0
	|			ТОГДА 1
	|		ИНАЧЕ МаксВесОбъем.Коэффициент
	|	КОНЕЦ * ЕСТЬNULL(МаксВесОбъем.Объем, 0),
	|	ТоварыРаспоряжений.Номенклатура,
	|	ТоварыРаспоряжений.Характеристика,
	|	ТоварыРаспоряжений.Серия";
	...
	Пока Результат.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Результат.ВесВГХ) Тогда 
			ПоказательВеса = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрРазделить(Результат.ВесВГХ," ")[1]); 
			СтрокаВГХ.Вес  = Результат.КоличествоШтук * ?(ПоказательВеса <> Неопределено, ПоказательВеса, 1);		
		КонецЕсли; 
		...
		Если ЗначениеЗаполнено(Результат.Высота) И ЗначениеЗаполнено(Результат.Ширина) И ЗначениеЗаполнено(Результат.Длина) Тогда
			...
			ПоказательДлины  = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрРазделить(Результат.Длина," ")[1]); 
			Если ПоказательВысоты <> Неопределено И ПоказательШирины <> Неопределено И ПоказательДлины <> Неопределено Тогда
				СтрокаВГХ.Объем = Результат.КоличествоШтук * ПоказательВысоты/1000 * ПоказательШирины/1000 * ПоказательДлины/1000;
			КонецЕсли; 	
		КонецЕсли;

	КонецЦикла;
    ТаблицаВГХ.Свернуть("Распоряжение,Ордер,Задание","КоличествоШтук,КоличествоМест,Вес,Объем"); 
...
//	Определяет установлена ли в строках заказов клиента печать сертификатов 
// 
// Параметры:
//	МассивОбъектов - Массив - Массив заказов клиента
//
Функция ВРаспоряженияхУказанаПечатьСертификатов(МассивОбъектов) Экспорт 
