	...
	Попытка
		...
		Для Каждого Настройка Из НастройкиОбмена Цикл 
			...
			Если ПолнаяСинхронизация Тогда
				...
				Иначе
					ОбъектыСинхронизации = ВыбратьОбъектыИБПоНастройке(Настройка.MetadataSynonym, СинхронизируемыеРеквизиты, Настройка.AttributeForName); 
...
//
Функция ВыбратьОбъектыИБПоНастройке(ПолноеИмяОбъектаМетаданных, СинхронизируемыеРеквизиты, РеквизитНаименования)
	...
	Если ЗначениеЗаполнено(РеквизитНаименования) Тогда 
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ОбъектМД.Наименование КАК Наименование", СтрШаблон("ОбъектМД.%1 КАК Наименование",РеквизитНаименования));		
	КонецЕсли;
