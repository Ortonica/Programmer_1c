...
Функция РСК_УстановитьЕдиницу(СтрокаТЧ)
	...
	Если СтрокаТЧ.Штрихкод <> "" Тогда
		...
		|	ВЫБОР
		|		КОГДА тШтрихкоды.ЕИ = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяССылка)
		|			ТОГДА ЕСТЬNULL(УпаковкаХранения.Ссылка, ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяССылка))
		|		ИНАЧЕ тШтрихкоды.ЕИ
		|	КОНЕЦ КАК ЕИ,
		//++РС Консалт: Минаков А.С. Задача 243264
		#КонецВставки
		...
		|	ГДЕ
		|		(&ВсяНоменклатура
		|				ИЛИ УпаковкиЕдиницыИзмерения.Владелец = &Номенклатура)
		...
		|	
		|	СГРУППИРОВАТЬ ПО
		|		УпаковкиЕдиницыИзмерения.Владелец) КАК УпаковкаХранения
		|	ПО тШтрихкоды.Номенклатура = УпаковкаХранения.Номенклатура"); 
		//++РС Консалт: Минаков А.С. Задача 243264
		#КонецВставки
