...
Процедура ЗаполнитьВодителейНаСервере()
	...
	Запрос.Текст = 
		...
		|	Контрагенты.ИНН КАК ИНН_Контрагента,
		|	ФизическиеЛица.ИНН КАК ИНН_ФизЛица,
...
Процедура ВодителиФизЛицоПриИзменении(Элемент)
	...
	Если ТД = Неопределено Тогда
		Возврат;
	КонецЕсли;
	СтруктураПартнёра = ПолучитьПартнёра(ТД.ФизЛицо);
	ЗаполнитьЗначенияСвойств(ТД, СтруктураПартнёра);
КонецПроцедуры
...
Функция ПолучитьПартнёра(ФизЛицо)
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(Контрагенты.Партнер, ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)) КАК Партнер,
		|	ЕСТЬNULL(Контрагенты.Ссылка, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)) КАК Контрагент,
		|	ЕСТЬNULL(Контрагенты.ИНН, """") КАК ИНН_Контрагента,
		|	ФизическиеЛица.ИНН КАК ИНН_ФизЛица,
		|	ФизическиеЛица.РСК_ОсновноеТранспортноеСредство КАК ТранспортноеСредство
		|ИЗ
...
//e1cib/data/Документ.Задание?ref=bfbf64d6d4cc63694e1448a1e549df0b&НаКлиенте
&НаКлиенте
Процедура ВодителиКонтрагентПриИзменении(Элемент)
	ТД = Элементы.Водители.ТекущиеДанные;
	Если ТД = Неопределено Тогда
		Возврат;
	КонецЕсли;
	СтруктураКонтрагента = ПолучитьСтруктуруКонтрагента(ТД.Контрагент);
	ЗаполнитьЗначенияСвойств(ТД, СтруктураКонтрагента);
КонецПроцедуры

//++ РС Консалт: Трофимов Евгений 07.02.2023 Тикет 23697
//e1cib/data/Документ.Задание?ref=bfbf64d6d4cc63694e1448a1e549df0b
&НаСервереБезКонтекста
Функция ПолучитьСтруктуруКонтрагента(Контрагент)
...
КонецФункции // ПолучитьСтруктуруКонтрагента()

//++ РС Консалт: Трофимов Евгений 07.02.2023 Тикет 23697
//e1cib/data/Документ.Задание?ref=bfbf64d6d4cc63694e1448a1e549df0b
&НаКлиенте
Процедура ИНН_ПриИзменении(Элемент)
	...
	ЗаписатьИНН(ТД.Контрагент, ТД.ФизЛицо, ТД.ИНН_ФизЛица);
	ПоказатьОповещениеПользователя("ИНН записан");
	Если Элемент.Имя = "ВодителиИНН_ФизЛица" Тогда
		ТД.ИНН_Контрагента = ТД.ИНН_ФизЛица;
	Иначе
		ТД.ИНН_ФизЛица = ТД.ИНН_Контрагента;
	КонецЕсли;

КонецПроцедуры // ИНН_ПриИзменении()

//++ РС Консалт: Трофимов Евгений 07.02.2023 Тикет 23697
//e1cib/data/Документ.Задание?ref=bfbf64d6d4cc63694e1448a1e549df0b
&НаСервереБезКонтекста
Процедура ЗаписатьИНН(Контрагент, ФизЛицо, ИНН)

	УстановитьПривилегированныйРежим(Истина);
	Если ЗначениеЗаполнено(Контрагент) Тогда
		РСК_ВызовСервера.ИзменитьЗначениеРеквизита(Контрагент,"ИНН",ИНН);
	КонецЕсли;
	Если ЗначениеЗаполнено(ФизЛицо) Тогда
		РСК_ВызовСервера.ИзменитьЗначениеРеквизита(ФизЛицо,"ИНН",ИНН);
	КонецЕсли;

КонецПроцедуры // ЗаписатьИНН()
