...
Процедура ЗаполнитьВодителейНаСервере()
	...
	Запрос.Текст = 
		...
		|	ФизическиеЛица.РСК_ОсновноеТранспортноеСредство КАК ТранспортноеСредство
		|ИЗ
...
Процедура ВодителиФизЛицоПриИзменении(Элемент)
	...
	Если ТД = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если ЗначениеЗаполнено(ТД.ФизЛицо) Тогда
		ТД.ТранспортноеСредство = РСК_ВызовСервера.ЗначениеРеквизитаОбъекта(ТД.ФизЛицо, "РСК_ОсновноеТранспортноеСредство");
	Иначе
		ТД.ТранспортноеСредство = Неопределено;
	КонецЕсли;
КонецПроцедуры

//++ РС Консалт: Трофимов Евгений 07.02.2023 Тикет 23697
//e1cib/data/Документ.Задание?ref=bfbf64d6d4cc63694e1448a1e549df0b
&НаКлиенте
Процедура ВодителиТранспортноеСредствоПриИзменении(Элемент)
	ТД = Элементы.Водители.ТекущиеДанные;
	Если ТД = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ТД.ФизЛицо) Тогда
		Возврат;
	КонецЕсли;
	РСК_ВызовСервера.ИзменитьЗначениеРеквизита(ТД.ФизЛицо, "РСК_ОсновноеТранспортноеСредство", ТД.ТранспортноеСредство, Истина); 
КонецПроцедуры
...
Процедура ПроверкиПередЗаполнениемТарифов(Отказ)
	...
	Для Каждого Стр Из Водители Цикл
		...
		Если Не ЗначениеЗаполнено(Стр.ФизЛицо) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не указан водитель",,"Водители["+Формат(НомерСтроки-1,"ЧГ=0")+"].ФизЛицо",,Отказ);
		КонецЕсли;
	КонецЦикла;
...
Процедура СоздатьУстановкиЦен()
	...
	Для Каждого Стр Из Водители Цикл
		...
		Результат = СоздатьУстановкуЦенНаСервере(Стр.ФизЛицо, АдресХранилища);
		НС = УстановкиЦен.Добавить();
		ЗаполнитьЗначенияСвойств(НС, Результат);
		НС.Водитель = Стр.ФизЛицо;
	КонецЦикла;
...
//
// Параметры:
//  Водитель		 - СправочникСсылка.ФизическиеЛица - Водитель
...
Функция СоздатьУстановкуЦенНаСервере(Водитель, АдресХранилища)
	...
	ВидЦеныПоставщика = Справочники.РСК_ВидыЦенФизЛиц.НайтиПоНаименованию(ИмяВидаЦен,Истина,,Водитель);
	Если НЕ ЗначениеЗаполнено(ВидЦеныПоставщика) Тогда
		оСпр = Справочники.РСК_ВидыЦенФизЛиц.СоздатьЭлемент();
...
//e1cib/data/Документ.Задание?ref=abf6db76aafd7de84ef44e63a771c2e5
&НаКлиенте
Процедура ДоставленныеТСРНоменклатураПриИзменении(Элемент)
	ТД = Элементы.ДоставленныеТСР.ТекущиеДанные;
	Если ТД = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТД.Номенклатура) Тогда
		ТД.ВидНоменклатуры = РСК_ВызовСервера.ЗначениеРеквизитаОбъекта(ТД.Номенклатура, "ВидНоменклатуры");
		ТД.ИмяТарифа = ПолучитьИмяТарифа(ТД.Номенклатура);
	Иначе
		ТД.ВидНоменклатуры = Неопределено;
		ТД.ИмяТарифа = "";
	КонецЕсли;
	
	ДоставленныеТСРИмяТарифаПриИзменении(Неопределено);
	
КонецПроцедуры

//++ РС Консалт: Трофимов Евгений 13.02.2023 Задача 23088
//e1cib/data/Документ.Задание?ref=abf6db76aafd7de84ef44e63a771c2e5
&НаСервереБезКонтекста
Функция ПолучитьИмяТарифа(Номенклатура)

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(РСК_НоменклатураИТарифыДоставкиТСР.ИмяТарифа, ПРЕДСТАВЛЕНИЕ(спрНоменклатура.ВидНоменклатуры)) КАК ИмяТарифа
		|ИЗ
		|	Справочник.Номенклатура КАК спрНоменклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РСК_НоменклатураИТарифыДоставкиТСР КАК РСК_НоменклатураИТарифыДоставкиТСР
		|		ПО спрНоменклатура.Ссылка = РСК_НоменклатураИТарифыДоставкиТСР.Номенклатура
		|ГДЕ
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.ИмяТарифа;
	КонецЕсли;
	Возврат "";

КонецФункции // ПолучитьИмяТарифа()
