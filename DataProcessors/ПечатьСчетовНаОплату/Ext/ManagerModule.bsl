...
Процедура РСК_ЗаполнитьРеквизитыПодвалаСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, СоответствиеСтавокНДС, ПараметрыПечати)
	...
	Если ОтображатьФаксимиле Тогда
		...
		ПечатьОрганизации = ПолучитьПечатьИлиПодпись(ДанныеПечати.Ссылка.Организация);
		...
		//Если ЗначениеЗаполнено(ПечатьОрганизации) Тогда
		//	
		//	ПутьКФайлу = СтрШаблон("%1%2",ПечатьОрганизации[0].Каталог, ПечатьОрганизации[0].ПутьКФайлу);
		//	
		//	ДанныеПечати = Новый Картинка(ПутьКФайлу,Истина);
		//	
		//	Попытка
		//		Печать_Рисунок = Область.Рисунки["ПечатьОрганизации"];
		//	Исключение
		//	КонецПопытки;
		//	Попытка
		//		Печать_Рисунок = Область.Рисунки["ПечатьОрганизации1"];
		//	Исключение
		//	КонецПопытки;
		//	Попытка
		//		Печать_Высота = Печать_Рисунок.Высота;
		//		Печать_Ширина = Печать_Рисунок.Ширина;
		//		Печать_Рисунок.Картинка = ДанныеПечати;
		//	Исключение
		//	КонецПопытки;
		//	
		//Иначе
		//	
		//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Печать организации не найдена");
		//	
		//КонецЕсли;
	КонецЕсли;
	#КонецВставки
