...
Функция ВыгрузитьНаСервере()
	...
	Для Каждого стрРез из тзРез Цикл
		...
		//ПровТЗ = ПолучитьТЗ();
		//Если ПровТЗ = Неопределено Тогда 
		//	ЗаявительТСР = Справочники.Партнеры.ПустаяСсылка();
		//Иначе
		//	парамОтбора = Новый Структура("НомПП", стрРез.НомерСтроки);
		//	Найдено = ПровТЗ.НайтиСтроки(парамОтбора);
		//	Если Найдено.Количество()>0 И Найдено[0].Использовать Тогда
		//		ЗаявительТСР = Найдено[0].Ссылка;
		//	Иначе
		//		ЗаявительТСР = Справочники.Партнеры.ПустаяСсылка();
		//	КонецЕсли;
		//КонецЕсли;
		...
		//Если НЕ ЗначениеЗаполнено(ЗаявительТСР) Тогда
			...
			//поиск Заявителя по номеру СНИЛС (Код справочника ПолучателиТСР)
			// приведём СНИЛСЗаявителя к нормальному значению
			СНИЛС = СокрЛП(СтрЗаменить(СтрЗаменить(стрРез.СНИЛС,"-","")," ",""));
			...
			//	|	И Партнеры.РСК_СНИЛС = &СНИЛС
			//	|	И НЕ Партнеры.ПометкаУдаления
			//	|	И Партнеры.Родитель = &ПартнёрФНС";
			//
			//Запрос.УстановитьПараметр("СНИЛС", СНИЛС);
			//Запрос.УстановитьПараметр("ПартнёрФНС", ПартнёрФНС);
			//Выборка = Запрос.Выполнить().Выбрать();
			//Пока Выборка.Следующий() Цикл
			//	ЗаявительТСР = Выборка.Ссылка;
			//КонецЦикла;
			...
			Иначе
				...
				Если ЗначениеЗаполнено(ДовЛицо) Тогда
					...
					Если ПерезаписываемыеДанные.Найти("ПаспортныеДанныеДовЛица") <> Неопределено Тогда
						оДовЛицо.ДополнительнаяИнформация = ?(СтрРез.Свойство("ПаспортныеДанныеДовЛица"),стрРез.ПаспортныеДанныеДовЛица,"");
					КонецЕсли;
				Иначе
					...
					Если СтрРез.Свойство("АдресДовЛица") Тогда
						ПерезаписатьКонтакт(
							оДовЛицо,
							Перечисления.ТипыКонтактнойИнформации.Адрес,
							РСК_ПовтИсп.Получить_Справочники_ВидыКонтактнойИнформации_АдресКонтактногоЛица(),
					...
					КонецЕсли;
					Если СтрРез.Свойство("ТелефонДовЛица") Тогда
						ПерезаписатьКонтакт(
							оДовЛицо,
							Перечисления.ТипыКонтактнойИнформации.Телефон,
							Справочники.ВидыКонтактнойИнформации.ТелефонКонтактногоЛица,
