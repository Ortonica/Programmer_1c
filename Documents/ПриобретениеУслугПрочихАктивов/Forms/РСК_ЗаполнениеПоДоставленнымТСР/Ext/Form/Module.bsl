...
Процедура ЗаполнитьДокументНаСервере(ДанныеФормы)
	...
	оПУПА.Комментарий = "Услуги водителя "+ВодительФизЛицо+" по доставке ТСР за периорд с "+Формат(Период.ДатаНачала,"ДФ=dd.MM.yyyy") + " по " + Формат(Период.ДатаОкончания,"ДФ=dd.MM.yyyy");
	...
	Для Каждого Стр Из оПУПА.Расходы Цикл
		Если оПУПА.ЦенаВключаетНДС Тогда
...
Функция ТекстЗапроса()
	...
	Текст = 
		...
		|	МАКСИМУМ(ЕСТЬNULL(РСК_ВидыЦенФизЛиц.Ссылка, ЗНАЧЕНИЕ(Справочник.РСК_ВидыЦенФизЛиц.ПустаяСсылка))) КАК ВидЦен,
		|	ФизическиеЛица.Ссылка КАК Партнер
		|ПОМЕСТИТЬ Доставки
		|ИЗ
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица КАК ФизическиеЛица
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РСК_ВидыЦенФизЛиц КАК РСК_ВидыЦенФизЛиц
		|			ПО ФизическиеЛица.Ссылка = РСК_ВидыЦенФизЛиц.Владелец
		|				И ФизическиеЛица.Наименование = РСК_ВидыЦенФизЛиц.Наименование
		|		ПО (ИСТИНА)
		|ГДЕ
		|	ФизическиеЛица.Ссылка = &ВодительФизЛицо
		|
		|СГРУППИРОВАТЬ ПО
		...
		|		ПО Доставки.Услуга = ДатыАктуальныхЦен.Номенклатура
		...
		|		ПО Доставки.Услуга = ЦеныНоменклатурыПоставщиков.Номенклатура
