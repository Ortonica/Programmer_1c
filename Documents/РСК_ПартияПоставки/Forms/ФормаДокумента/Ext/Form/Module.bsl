...
Процедура ПослеВопросаОСозданииПриобретений(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ШапкаЗаказа = ПолучитьСтруктуруГлавногоЗаказа(Объект.Ссылка);
	...
	Для Каждого Стр Из Объект.Контейнеры Цикл
		...
		ПараметрыПриобретения.Вставить("ШапкаЗаказа", ШапкаЗаказа);
...
//  Назначение главного заказа по максимальному объёму товара
//
// Параметры:
//  ПартияПоставки	 - ДокументСсылка.РСК_ПартияПоставки - Партия поставки
// 
// Возвращаемое значение:
//   - Структура
//
&НаСервереБезКонтекста
Функция ПолучитьСтруктуруГлавногоЗаказа(ПартияПоставки)

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	РСК_ПартияПоставкиТовары.ЗаказПоставщику КАК ЗаказПоставщику,
		|	СУММА(РСК_ПартияПоставкиТовары.Количество * РСК_ПартияПоставкиТовары.Объем) КАК Объем
		|ПОМЕСТИТЬ ОбъёмыЗаказов
		|ИЗ
		|	Документ.РСК_ПартияПоставки.Товары КАК РСК_ПартияПоставкиТовары
		|ГДЕ
		|	РСК_ПартияПоставкиТовары.Ссылка = &Ссылка
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МАКСИМУМ(ОбъёмыЗаказов.Объем) КАК Объем
		|ПОМЕСТИТЬ МаксимальныйОбъём
		|ИЗ
		|	ОбъёмыЗаказов КАК ОбъёмыЗаказов
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	ОбъёмыЗаказов.ЗаказПоставщику КАК ЗаказПоставщику,
		|	ОбъёмыЗаказов.Объем КАК Объем,
		...
		|ИЗ
		|	ОбъёмыЗаказов КАК ОбъёмыЗаказов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ МаксимальныйОбъём КАК МаксимальныйОбъём
		|		ПО ОбъёмыЗаказов.Объем = МаксимальныйОбъём.Объем
		...
		|		ПО ОбъёмыЗаказов.ЗаказПоставщику = докЗаказПоставщику.Ссылка
		|";
	
	Запрос.УстановитьПараметр("Ссылка", ПартияПоставки);
	Возврат РСК_ВызовСервера.ЗапросВСтруктуру(Запрос);

КонецФункции // ПолучитьСтруктуруГлавногоЗаказа()
...
Процедура ПровестиПриобретение(ПараметрыПриобретения)
	...
	тзТовары = РезультатыЗапроса[3].Выгрузить();
	...
	ДанныеЗаполнения.Вставить("СкладПоступления", ПараметрыПриобретения.ШапкаЗаказа.Склад);
	ДанныеЗаполнения.Вставить("РеквизитыШапки", ПараметрыПриобретения.ШапкаЗаказа);
	...
	ЗаполнитьЗначенияСвойств(оДок, ПараметрыПриобретения.ШапкаЗаказа); //Заполнились поля:
	...
	Для Каждого СтрокаТоваров Из тзТовары Цикл
		...
		КонецЕсли;
		НовСтр.ОбъектРасчетов = СтрокаТоваров.ЗаказПоставщику;
