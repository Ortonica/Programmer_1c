...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		...
		Запрос.Текст = 
			...
			|	ЗаказПоставщикуТовары.ДатаЕТА КАК ДатаЕТА
			|ИЗ
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	//Проверка  ДатаЗагругкиКонтейнера >= ДатаЕТА >= ДатаПоступления
	Если ДатаЕТА = '0001.01.01' Тогда
		Для Каждого Стр Из Контейнеры Цикл
			Если Стр.ДатаЗагрузкиКонтейнера > '0001.01.01' И Стр.ДатаПоступленияНаСклад > '0001.01.01' Тогда
				Если Стр.ДатаЗагрузкиКонтейнера > Стр.ДатаПоступленияНаСклад Тогда
					ОбщегоНазначения.СообщитьПользователю(
						"Дата загрузки контейнера больше даты поступления на склад",,
						СтрШаблон("Контейнеры[%1].ДатаПоступленияНаСклад",Формат(Стр.НомерСтроки-1, "ЧГ=0")),
						"Объект",
						Отказ
					);
				КонецЕсли;
	...
	Иначе
		Для Каждого Стр Из Контейнеры Цикл
			Если Стр.ДатаЗагрузкиКонтейнера > '0001.01.01' Тогда
				Если Стр.ДатаЗагрузкиКонтейнера > ДатаЕТА Тогда
					ОбщегоНазначения.СообщитьПользователю(
						"Дата загрузки контейнера больше даты ЕТА",,
						СтрШаблон("Контейнеры[%1].ДатаЗагрузкиКонтейнера",Формат(Стр.НомерСтроки-1, "ЧГ=0")),
						"Объект",
						Отказ
					);
				КонецЕсли;
			КонецЕсли;
			Если Стр.ДатаПоступленияНаСклад > '0001.01.01' Тогда
				Если ДатаЕТА > Стр.ДатаПоступленияНаСклад Тогда
					ОбщегоНазначения.СообщитьПользователю(
						"Дата ЕТА больше даты поступления на склад",,
						СтрШаблон("Контейнеры[%1].ДатаПоступленияНаСклад",Формат(Стр.НомерСтроки-1, "ЧГ=0")),
						"Объект",
						Отказ
					);
