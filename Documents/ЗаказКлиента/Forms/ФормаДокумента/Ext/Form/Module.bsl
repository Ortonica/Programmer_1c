...
Процедура РСК_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	//РСК+ Кикинева А.Г. добавление на страницу Самовывоз доп.данных 
	НовыйЭлемент = Элементы.Добавить("ДополнительнаяИнформацияПоДоставке4", Тип("ПолеФормы"), Элементы.СтраницаСамовывоз);
	НовыйЭлемент.Заголовок = "Дополнительная "+Символы.ПС	+
	"информация по доставке";
	НовыйЭлемент.ПоложениеЗаголовка=ПоложениеЗаголовкаЭлементаФормы.Лево;
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.ДополнительнаяИнформацияПоДоставке"; 
	НовыйЭлемент.Ширина = 59; 
	НовыйЭлемент.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Лево;
	НовыйЭлемент.РастягиватьПоГоризонтали = Ложь; 
	Элементы.СтраницаСамовывоз.Группировка=ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
    //РСК- Кикинева А.Г. добавление на страницу Самовывоз доп.данных 
	
	//++ Конарев И.И. Проведение заказов с ПДЗ
	НовыйЭлемент = Элементы.Добавить("ИгнорироватьПДЗ", Тип("ПолеФормы"), Элементы.ГруппаПараметрыПраво);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ПутьКДанным = "Объект.РСК_ИгнорироватьПДЗ";
	НовыйЭлемент.Видимость   = Пользователи.РолиДоступны("РСК_ПравоСнятьЗапретЗадолженностиВДоговоре",Пользователи.ТекущийПользователь());
	//--
	
	//РСК+ Кикинева А.Г. задача 243874 запрет на редактирование даты отгрузки и адреса доставки 
	//после установки статуса Отгрузить  
	...
	Запрос.УстановитьПараметр("Состояние", Перечисления.СостоянияЗаказовКлиентов.ВПроцессеОтгрузки); 
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий()  Тогда
		Если НЕ Пользователи.РолиДоступны("РСК_ПроведениеЗаказаВСтатусеКОтгрузке",Пользователи.ТекущийПользователь()) Тогда  
			...
			Элементы.АдресДоставкиПолучателя3.ТолькоПросмотр=Истина;
			...
			Иначе
				Для каждого Строка из Объект.Товары Цикл
					Если Строка.ВариантОбеспечения=Перечисления.ВариантыОбеспечения.Отгрузить Тогда
						Элементы.ТоварыДатаОтгрузки.ТолькоПросмотр=Истина;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли 
		КонецЕсли;
	КонецЕсли;
	//РСК- Кикинева А.Г. задача 243874
КонецПроцедуры
...
Процедура РСК_ТоварыВыборПосле(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле = Элементы.ТоварыРСК_ДоступноАналогов И Элементы.Товары.ТекущиеДанные <> Неопределено Тогда
		
		Стр = Элементы.Товары.ТекущиеДанные;
		ОткрытьФорму("Документ.ЗаказКлиента.Форма.РСК_ФормаАналогов", Новый Структура("Номенклатура, Характеристика, Склад, Подразделение", Стр.Номенклатура, Стр.Характеристика, ?(ЗначениеЗаполнено(Стр.Склад), Стр.Склад, Объект.Склад), Объект.Подразделение), ЭтаФорма,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Функция ТаблицаЗначенийВМассив(ТаблицаЗначений)
	...
	//++ РС Консалт: Минаков А.С. Задача 20226
	Массив = Новый Массив();
	СтруктураСтрокой = "Номенклатура,Характеристика,Серия,ВариантОбеспечения,КоличествоУпаковок,Упаковка";
...
Функция РСК_РассчитатьВГХ(ПсевдоТаблицаТовары) 
	...
	Если ТаблицаТовары.Количество() Тогда
		Возврат РСК_ВызовСервера.РассчитатьВесогабаритныеХарактеристикиДокумента(ТаблицаТовары);
	Иначе
		Возврат Новый Структура("РСК_Вес,РСК_Кубатура","0/0","0/0"); 
	КонецЕсли;
	//--  
...
Процедура РСК_РеквизитыПечати(Команда)
	...
	#Вставка 
	СтруктураПараметров.Вставить("ТипыСертификатовКПечати",		   Объект.РСК_СертификатыКПечати);
	#КонецВставки
...
&НаКлиенте
&ИзменениеИКонтроль("ТоварыСерияПриИзменении")
Процедура РСК_ТоварыСерияПриИзменении(Элемент)
...
Процедура РСК_ТоварыНоменклатураПриИзменении(Элемент)
...
&НаСервере
Процедура РСК_СделкаПриИзмененииПослеНаСервере()
	//++ Конарев И.И. Задача 249200 
	ВидСделкиПредставление = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Сделка,"ВидСделки.Наименование"); 
	Если ЗначениеЗаполнено(ВидСделкиПредставление) И ВидСделкиПредставление = "Мы - соисполнители" Тогда 
		НаправлениеСоисполнительство = Справочники.НаправленияДеятельности.НайтиПоНаименованию("Соисполнительство"); 
		Если ЗначениеЗаполнено(НаправлениеСоисполнительство) Тогда
			Объект.НаправлениеДеятельности = НаправлениеСоисполнительство;
...
&НаКлиенте
Процедура РСК_СделкаПриИзмененииПосле(Элемент)
	Если ЗначениеЗаполнено(Объект.Сделка) Тогда
		РСК_СделкаПриИзмененииПослеНаСервере(); 
	КонецЕсли;
КонецПроцедуры

&НаСервере
&ИзменениеИКонтроль("ХозяйственнаяОперацияПриИзмененииСервер")
Процедура РСК_ХозяйственнаяОперацияПриИзмененииСервер(ВызовПоКоманде)
	...
	Если ОперацииПередачи.Найти(Объект.ХозяйственнаяОперация) <> Неопределено Тогда
		...
		Если (ИспользоватьРучныеСкидкиВПродажах Или ИспользоватьАвтоматическиеСкидкиВПродажах) 
			И НЕ Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи Тогда
		#КонецВставки
...
Процедура РСК_УстановитьВидимостьЭлементовПоОперацииСервер()
	...
	#Вставка 
	//++ Конарев И.И. задача 257831: расчет скидок для хоз.операции "передача на хранение с правом продажи"
	ЭтоПередачаТоваровБезПраваПродажи = Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию;
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(Элементы, МассивЭлементов, "Видимость", Не (ЭтоПередачаТоваровБезПраваПродажи ИЛИ ЭтоПоставкаПодПринципала));
	#КонецВставки
