...
Процедура ВыполнитьСкладскуюОперациюПоШтрихкоду(ШК)
	...
	ИначеЕсли НайденнаяОперация = ПредопределенноеЗначение("Справочник.РСК_СкладскиеОперации.ЗавершеноСОшибками") Тогда
		Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыОтборовРазмещенийТоваров.ВыполненоСОшибками");
		Модифицированность = Истина;
		СтатусПриИзмененииСервер();
	ИначеЕсли НайденнаяОперация = ПредопределенноеЗначение("Справочник.РСК_СкладскиеОперации.ВРаботе") Тогда
