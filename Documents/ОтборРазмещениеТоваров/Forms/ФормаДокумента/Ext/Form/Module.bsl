...
//РСК+ Кикинева А.Г. задача 261415 заполнение Исполнителя на странице Исполнители 
&НаСервере
Процедура ЗаполнитьИсполнителяНаСервере()  
	
	Если ЗначениеЗаполнено(Объект.Исполнитель)
		И Не Объект.Исполнители.Количество() Тогда
		НовыйИсполнитель = Объект.Исполнители.Добавить();
		НовыйИсполнитель.Исполнитель=Справочники.Пользователи.НайтиПоНаименованию(Объект.Исполнитель); 
	КонецЕсли;       
