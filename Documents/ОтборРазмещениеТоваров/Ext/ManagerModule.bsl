...
Функция РСК_ПечатьЗадания(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		ОбластьСкладИсполнитель.Параметры.Получатель = ПолучитьПолучателя(ВыборкаПоДокументам.Ссылка);
		//-- КонецЗадачи 23087
		#КонецВставки
		...
		//++ РС Консалт: Трофимов Евгений 20.01.2023 Задача 23154
		//e1cib/data/Документ.Задание?ref=bcb44edb8528e94044a02d443a51d096
		ОсобыеОтметки = ПолучитьОсобыеОтметки(ВыборкаПоДокументам.Ссылка);
		Если НЕ ПустаяСтрока(ОсобыеОтметки) Тогда
			ОбластьОсобыеОтметки.Параметры.ОсобыеОтметки = ОсобыеОтметки;
...
//++ РС Консалт: Трофимов Евгений 19.01.2023 Задача 23087
//e1cib/data/Документ.Задание?ref=a70cc94fd7f4d8874de8888e78f9c280
Функция ПолучитьПолучателя(ДокументОтбор)
	...
	Если ТипЗнч(Выборка.Распоряжение) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Распоряжение, "Контрагент");
	ИначеЕсли ТипЗнч(Выборка.Распоряжение) = Тип("ДокументСсылка.ЗаказНаПеремещение") Тогда
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Распоряжение, "СкладПолучатель");
	Иначе
		Возврат "";
	КонецЕсли;
...
//Шамрай Вероника: менеджер по продажам в заке клиента на вкладке "Доставка" должен поставить галку 
//"Обратить внимание транспортной службы (для склада)" и в текстовом поле указать необходимую информацию. 
//Эту информацию нужно выводить в листе отбора
//++ РС Консалт: Трофимов Евгений 20.01.2023 Задача 23154
//e1cib/data/Документ.Задание?ref=bcb44edb8528e94044a02d443a51d096
Функция ПолучитьОсобыеОтметки(ДокументОтбор)
	...
	Если ТипЗнч(Выборка.Распоряжение) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		Если СтруктураОсобыхПеревозок.ОсобыеУсловияПеревозки Тогда
			Возврат СтруктураОсобыхПеревозок.ОсобыеУсловияПеревозкиОписание;
		КонецЕсли;
	КонецЕсли;
