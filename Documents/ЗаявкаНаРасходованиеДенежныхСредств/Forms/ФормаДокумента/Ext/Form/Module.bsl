...
Функция СформироватьНазначениеПлатежа(СтруктураПараметров) Экспорт
	...
	Если РП.Количество() Тогда		
		...
		Для Каждого СтрокаТч Из РП Цикл
			...
			ИначеЕсли СтрокаТч.СтавкаНДС = Справочники.СтавкиНДС.БезНДС Тогда
...
&НаСервереБезКонтекста
Функция ИдентификаторОбъектаМетаданных(Тип)
	Возврат ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Тип);	
КонецФункции

&НаКлиенте
&ИзменениеИКонтроль("РасшифровкаПлатежаВыбор")
Процедура РСК_РасшифровкаПлатежаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	//-- НЕ УТ
	#Вставка
	Если Поле.Имя = "РасшифровкаПлатежаОбъектРасчетов" И Элемент.ТолькоПросмотр Тогда
		Если Элемент.ТекущиеДанные <> Неопределено Тогда
			ОбработатьНажатиеОбъектРасчетов(Элемент.ТекущиеДанные.ОбъектРасчетов); 
		КонецЕсли;
	КонецЕсли;
	#КонецВставки
	Возврат;
...
&НаКлиенте
Процедура ОбработатьНажатиеОбъектРасчетов(ОбъектРасчетов)
	
	ТипОбъекта = ЗначениеРеквизитаОбъекта(ОбъектРасчетов,"ТипСсылки");
	Если ТипОбъекта = ИдентификаторОбъектаМетаданных(Тип("ДокументСсылка.ЗаказПоставщику")) Тогда
		ОткрытьФорму("Документ.ЗаказПоставщику.Форма.ФормаДокумента",Новый Структура("Ключ",ЗначениеРеквизитаОбъекта(ОбъектРасчетов,"Объект")));		
	КонецЕсли; 
