...
#Область ОтражениеУслугСоисполнителей  

&После("ПриЗаписи")
Процедура РСК_ПриЗаписи(Отказ)

	Если НЕ ПометкаУдаления Тогда
		Если САВ_Услуги.Количество() > 0 Тогда
			ПроведениеДокументов.СформироватьДвиженияОтраженияУслугСоисполнителя(Ссылка, Отказ);
		КонецЕсли;
	Иначе
		ПроведениеДокументов.ОчиститьДвиженияЗадания(Ссылка, Отказ);		
	КонецЕсли;
