...
Функция РСК_ПечатьРасходногоОрдераНаТовары(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ИспользованиеСерий Из ДеревоОбъектов.Строки Цикл
		...
		Для Каждого ВыборкаПоДокументам Из ИспользованиеСерий.Строки Цикл
			...
			ВсегоНаименований = 0;
            #Вставка
			//РСК+ Кикинева А.Г Задача 256123
            ИтогоКоличество=0;  
					...
					Пока ВыборкаПоНоменклатуреТЧ.Следующий() Цикл
						...
						Пока ВыборкаПоХарактеристикамТЧ.Следующий() Цикл
			                ...
			                //РСК+ Кикинева А.Г Задача 256123
							    ...
							    ИтогоКоличествоУпаковок=ИтогоКоличествоУпаковок+ВыборкаПоСтрокамТЧ.КоличествоУпаковок;
							//РСК- Кикинева А.Г Задача 256123
                            #КонецВставки
			...
			ОбластьПодвалТаблицы.Параметры.ИтогоКоличествоУпаковок=ИтогоКоличествоУпаковок;   
			//РСК- Кикинева А.Г Задача 256123  
			#КонецВставки
            ...
            #Вставка
			//РСК+ Кикинева А.Г Задача 256123
			ОбластьПодписей.Параметры.ДатаПечати="Дата печати: "+ТекущаяДата();
			//РСК- Кикинева А.Г Задача 256123  
			#КонецВставки
...
//Фоновое задание вызывается из Документ.ЗаданиеНаПеревозку.Форма.РСК_ИзменениеСтатусовРО_ПриИсключенииИзМЛ
//++ РС Консалт: Трофимов Евгений 09.02.2023 Тикет 23385
//e1cib/data/Документ.Задание?ref=a4138b831ba66f9148449511f2e680ff
Процедура ИзменениеСтатусовРОприИсключенииИзМЛ(СтруктураПараметров, АдресВремХранилища = "") Экспорт

	СписокРезультатов = СтруктураПараметров.СписокРезультатов;
	СтруктураОтвета = Новый Структура;
	
	ё = 0;
    ПрошлыйПроцентВыполнения = 0;
	Для Каждого РО Из СтруктураПараметров.МассивИсключаемыхРО Цикл
		ё = ё + 1;
		ПроцентВыполнения = ё * 100 / СтруктураПараметров.МассивИсключаемыхРО.Количество();
        Если ПрошлыйПроцентВыполнения <> Окр(ПроцентВыполнения) Тогда
            ПрошлыйПроцентВыполнения = Окр(ПроцентВыполнения);
            ДлительныеОперации.СообщитьПрогресс(ПрошлыйПроцентВыполнения, ""+ё+" из "+СтруктураПараметров.МассивИсключаемыхРО.Количество());
        КонецЕсли;
		
		оРО = РО.ПолучитьОбъект();
		оРО.Статус = Перечисления.СтатусыРасходныхОрдеров.КОтгрузке;
		оРО.ОтгрузкаПоЗаданиюНаПеревозку = Ложь;
		оРО.ПорядокДоставки = "";
		оРО.ЗаданиеНаПеревозку = Неопределено;
		Если оРО.Проведен Тогда
			Попытка
				оРО.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
				Запись = СписокРезультатов.Добавить();
				Запись.Документ = РО;
				Запись.Результат = "Статус успешно изменён";
			Исключение
				Запись = СписокРезультатов.Добавить();
				Запись.Документ = РО;
				Запись.ЕстьОшибка = Истина;
				Запись.ОписаниеОшибки = ОписаниеОшибки();
				Запись.Результат = "Не удалось провести документ!";
			КонецПопытки;
		Иначе
			Попытка
				оРО.Записать(РежимЗаписиДокумента.Запись);
				Запись = СписокРезультатов.Добавить();
				Запись.Документ = РО;
				Запись.Результат = "Статус успешно изменён";
			Исключение
				...
				Запись.ЕстьОшибка = Истина;
				Запись.ОписаниеОшибки = ОписаниеОшибки();
				Запись.Результат = "Не удалось записать документ!";
			КонецПопытки;
		КонецЕсли;
	КонецЦикла;
	
	СтруктураОтвета.Вставить("СписокРезультатов", СписокРезультатов);
