...
Процедура РСК_ОтпустилПриИзмененииПосле(Элемент)
	...
	//+РС Консалт Назаров М.Ю. 28.03.2023 10:13:37
	ЗаполнитьДолжностьФизЛицаЕИС();
	//-РС Консалт Назаров М.Ю. 28.03.2023 10:13:37	
	
КонецПроцедуры  

&НаСервере
Процедура ЗаполнитьДолжностьФизЛицаЕИС()
	
	Если ЭтоГосконтрактЕИС Тогда 
		
		Объект.ОтпустилДолжность = "";
		
		ДолжностиФизЛица = РСК_Сервер.ПолучитьДолжностиФизЛицаПоОрганизацииЕИС(Объект.Отпустил, Объект.Организация);
		Если ДолжностиФизЛица.Количество() = 1 Тогда 
			Объект.ОтпустилДолжность = ДолжностиФизЛица[0];		
		КонецЕсли;   
