...
Функция РСК_СформироватьПечатнуюФормуМаршрутныйЛист(МассивОбъектов, ОбъектыПечати)
	...
	//-- КонецЗадачи 23089 
	//++ Конарев И.И. Пересчет ВГХ по характеристикам товаров
	ТаблицаВГХ = РСК_ВызовСервера.РассчитатьВГХМаршрутногоЛиста(МассивОбъектов);  
	Запрос.УстановитьПараметр("ТаблицаВГХ", ТаблицаВГХ);  
	//--
	#КонецВставки
	...
	РезультатШапкаДокументов = РезультатЗапросаПоДокументу[7].Выбрать();
	ВыборкаПоСсылкам         = РезультатЗапросаПоДокументу[8].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	СкладыПогрузкиВыборка    = РезультатЗапросаПоДокументу[9].Выбрать();
	ВыборкаПоОрдерам 		 = РезультатЗапросаПоДокументу[10].Выбрать();
	//-- КонецЗадачи 23089	
...
Функция ТекстЗапросаПФ_МаршрутныйЛист()
	...
	Текст = 
		...
		//|  
		|	ВЫБРАТЬ
		|	ТаблицаВГХ.Распоряжение КАК Распоряжение,
		|	ТаблицаВГХ.Ордер КАК Ордер,
		|	ТаблицаВГХ.Задание КАК Задание,
		|	ТаблицаВГХ.Вес КАК Вес,
		|	ТаблицаВГХ.Объем КАК Объем,
		|	ТаблицаВГХ.КоличествоШтук КАК КоличествоШтук,
		|	ТаблицаВГХ.КоличествоМест КАК КоличествоМест
		|ПОМЕСТИТЬ ВГХ
		|ИЗ
		|	&ТаблицаВГХ КАК ТаблицаВГХ 
		|;
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	РаспоряженияЗаданий.Ссылка КАК Ссылка,
...
Функция ПолучитьТаблицыСверкиСтатусовРО(Параметры) Экспорт
	...
	Запрос.Текст =
		...
		|	КСменеСтатуса.РО.ДатаОтгрузки КАК ДатаОтгрузки
		|ИЗ
		|	(ВЫБРАТЬ
		|		Сбор.РО КАК РО,
