...
Процедура РСК_МаршрутПередУдалениемПеред(Элемент, Отказ)
	...
	МассивИсключаемыхРО = ПолучитьМассивИсключаемыхРО(ВыделенныеСтроки);
...
//e1cib/data/Документ.Задание?ref=a4138b831ba66f9148449511f2e680ff
&НаСервере
Функция ПолучитьМассивИсключаемыхРО(ВыделенныеСтроки)
	...
	Запрос.УстановитьПараметр("ЗаданиеНаПеревозку", Объект.Ссылка);
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("РО");
