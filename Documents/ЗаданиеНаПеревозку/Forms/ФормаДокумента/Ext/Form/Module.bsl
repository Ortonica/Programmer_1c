...
Процедура РСК_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("РС_ФормаОткрытаПоШтрихкоду", РС_ФормаОткрытаПоШтрихкоду);
	Если РС_ФормаОткрытаПоШтрихкоду Тогда
...
Процедура РСК_ПечатьКомплектаДокументовВместо(Команда)
	ЗадатьВопросПечатьСертификатов("Все накладные");			
КонецПроцедуры

&НаКлиенте
Процедура РСК_ПечатьКомплектаДокументовВыделенныеВместо(Команда)
	ЗадатьВопросПечатьСертификатов("Только выделенные");	
КонецПроцедуры

&НаКлиенте
Процедура ПечатьКомплектаДокументовЗавершение(СписокРеализаций, ЮрЛица, ФизЛица, ПечататьСертификаты)
	...
	Если СписокРеализаций.Количество() Тогда
		...
		Если ЮрЛица.Количество() Тогда			
			...
			Если ПечататьСертификаты Тогда
				УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.РеализацияТоваровУслуг","СертификатыНоменклатурыИзображенияИзДокументов",ЮрЛица, ЭтаФорма);		
			КонецЕсли;
		КонецЕсли		
	КонецЕсли
	//++РС Консалт: Минаков А.С. Задача 21223
...
&НаКлиенте
Процедура РСК_МаршрутПередУдалениемПеред(Элемент, Отказ)
	//++ РС Консалт: Трофимов Евгений 09.02.2023 Тикет 23385
	//e1cib/data/Документ.Задание?ref=a4138b831ba66f9148449511f2e680ff
	//Корягина Юлия
	//1. В МЛ меняем статус с "Отправлен" на "Формируется". При этом статусы РО меняться не должны.
	//2. из списка заказов вытаскиваем тот заказ, который не уехал. При этом РО на товар должен отвязаться от МЛ и статус РО измениться на "К отгрузке"
	//3. Меняем обратно статус МЛ	
	ВыделенныеСтроки = Элементы.Маршрут.ВыделенныеСтроки;
	МассивИсключаемыхРО = ПолучитьМассивИсключаемыхРО(ВыделенныеСтроки, Отказ);
	Если МассивИсключаемыхРО.Количество() > 0 Тогда
		//Операция длительная. Выполним её в отдельном окне в отдельном потоке. 
		Парам = Новый Структура;
		Парам.Вставить("МассивИсключаемыхРО", МассивИсключаемыхРО);
		ОткрытьФорму("Документ.ЗаданиеНаПеревозку.Форма.РСК_ИзменениеСтатусовРО_ПриИсключенииИзМЛ",Парам,ЭтаФорма,Истина);
	КонецЕсли;
	//-- КонецТикета 23385	
КонецПроцедуры
...
//e1cib/data/Документ.Задание?ref=a4138b831ba66f9148449511f2e680ff
&НаСервере
Функция ПолучитьМассивИсключаемыхРО(ВыделенныеСтроки, Отказ = Ложь)
	
	Распоряжения = Новый Массив;
	//Нужно удалить этот устаревший выключатель из рабочей базы
	//Если НЕ РегистрыСведений.реа_ПредопределенныеЗначения.Значение("Менять статусы РО при удалении из МЛ", Истина) Тогда
	//	Возврат Распоряжения;
	//КонецЕсли;
	
	Для Каждого Идентификатор Из ВыделенныеСтроки Цикл
		ДанныеСтроки = Объект.Маршрут.НайтиПоИдентификатору(Идентификатор);
		НайденныеСтроки = Объект.Распоряжения.НайтиСтроки(Новый Структура("КлючСвязи",ДанныеСтроки.КлючСвязи));
		Для Каждого Стр Из НайденныеСтроки Цикл
			Распоряжения.Добавить(Стр.Распоряжение);
		КонецЦикла;
	КонецЦикла;
	...
	Запрос.Текст =
		...
		|	РасходныйОрдерНаТовары.Ссылка КАК РО,
		...
		|	И РасходныйОрдерНаТовары.ЗаданиеНаПеревозку = &ЗаданиеНаПеревозку
		|	И НЕ РасходныйОрдерНаТовары.ПометкаУдаления";
	Запрос.УстановитьПараметр("Распоряжения", Распоряжения);
	Запрос.УстановитьПараметр("ЗаданиеНаПеревозку", Объект.Ссылка);
	тзИсключаемыеРО = Запрос.Выполнить().Выгрузить();
	...
	Если НЕ РСК_ВызовСервера.ЕстьРоль("РСК_РедактированиеСкладскихОрдеровПрошлыхПериодов") Тогда
		Для Каждого Стр Из тзИсключаемыеРО Цикл
			Если Стр.ДатаОтгрузки < НачалоДня(ТекущаяДатаСеанса()) И Стр.Статус = Перечисления.СтатусыРасходныхОрдеров.Отгружен Тогда
				ОбщегоНазначения.СообщитьПользователю(""+Стр.РО+" находится в прошедшем периоде",Стр.РО,,,Отказ);
			КонецЕсли;
		КонецЦикла;
		Если Отказ Тогда
			ОбщегоНазначения.СообщитьПользователю("Запрещено снимать РО со статуса «Отгружен» в прошедшем периоде, поэтому удалять их из МЛ тоже запрещено (Тикет 849)");
			Возврат Новый Массив;
		КонецЕсли;
	КонецЕсли;
	//-- КонецТикета 24985	
	
	Возврат тзИсключаемыеРО.ВыгрузитьКолонку("РО");

КонецФункции // ПолучитьМассивИсключаемыхРО()
...
Процедура РСК_ОбработкаОповещенияПосле(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ПодключаемоеОборудование" Тогда
		Если ИмяСобытия = "ScanData" Тогда
			РСК_РаботаСоСканером.УстановитьВремяПоследнегоСобытия();
			СтруктураШК = МенеджерОборудованияКлиент.ПреобразоватьДанныеСоСканераВСтруктуру(Параметр);
			Если Лев(СтруктураШК.Штрихкод,2) = "20" Тогда // Это пользователь
				Объект.РСК_ОтветственныйЗаОтгрузку = РСК_РаботаСоСканером.ПользовательПоШК(СтруктураШК.Штрихкод);
				Если ЗначениеЗаполнено(Объект.РСК_ОтветственныйЗаОтгрузку) Тогда
					Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаданийНаПеревозку.Отправлено");
					Отказ = Ложь;
					ВыполнитьПроверкуПриСменеСтатусаНаСервере(Отказ);
					Если НЕ Отказ Тогда
						//РСК_СоздатьНакладные(Неопределено); (Накладные должен создавать другой отдел)
						Записать(Новый Структура("РежимЗаписи",РежимЗаписиДокумента.Проведение));
						ПодключитьОбработчикОжидания("ЗакрытьФормуПотом", 0.5, Истина);
					КонецЕсли;
				Иначе
					Сообщить("Не найден ответственный за отгрузку со штрихкодом "+СтруктураШК.Штрихкод);
				КонецЕсли;
			Иначе
				Сообщить("Неправильный штрихкод. Просканируйте штрихкод ответственного за отгрузку");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	//-- КонецЗадачи 23698
...
&НаКлиенте
Процедура РСК_ПриЗакрытииПосле(ЗавершениеРаботы)
	//++ РС Консалт: Трофимов Евгений 18.02.2023 Задача 23698
	//e1cib/data/Документ.Задание?ref=a1c07de7eccf0a5942be9c5a46bb066d
	Если РС_ФормаОткрытаПоШтрихкоду Тогда
...
//Вызывается по отложенному обработчику из процедуры ОбработкаОповещения(), 
//чтобы не сработал обработчик из общей формы "Авторизация по штрихкоду"
//++ РС Консалт: Трофимов Евгений 18.02.2023 Задача 23698
//e1cib/data/Документ.Задание?ref=a1c07de7eccf0a5942be9c5a46bb066d
&НаКлиенте
Процедура ЗакрытьФормуПотом()

	Закрыть();

КонецПроцедуры // ЗакрытьФормуПотом()

&НаКлиенте
Процедура РСК_СтатусПриИзмененииПосле(Элемент)
	//++ РС Консалт: Трофимов Евгений 20.02.2023 Задача 23698
	//e1cib/data/Документ.Задание?ref=a1c07de7eccf0a5942be9c5a46bb066d
	ВыполнитьПроверкуПриСменеСтатусаНаСервере();
	//-- КонецЗадачи 23698
КонецПроцедуры
...
//e1cib/data/Документ.Задание?ref=a1c07de7eccf0a5942be9c5a46bb066d
&НаСервере
Процедура ВыполнитьПроверкуПриСменеСтатусаНаСервере(Отказ = Ложь)

	Если НЕ РегистрыСведений.реа_ПредопределенныеЗначения.Значение("Функционал задачи № 243812", Истина) Тогда
		Возврат;
	КонецЕсли;
	
	Если Не СкладыСервер.ИспользоватьОрдернуюСхемуПриОтгрузке(Объект.Склад, Объект.Дата) Тогда
		Возврат;	
	КонецЕсли;
	
	Парам = Новый Структура;
	Задание = Новый ТаблицаЗначений;
	Задание.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("ДокументСсылка.ЗаданиеНаПеревозку"));
	Задание.Колонки.Добавить("Статус", Новый ОписаниеТипов("ПеречислениеСсылка.СтатусыЗаданийНаПеревозку"));
	Задание.Колонки.Добавить("РСК_ВыдачаТСР", Новый ОписаниеТипов("Булево"));
	Задание.Колонки.Добавить("Проведен", Новый ОписаниеТипов("Булево"));
	Задание.Колонки.Добавить("РСК_ОтветственныйЗаОтгрузку", Новый ОписаниеТипов("СправочникСсылка.Пользователи"));
	Задание.Колонки.Добавить("Склад", Новый ОписаниеТипов("СправочникСсылка.Склады"));
	
	НС = Задание.Добавить();
	НС.Ссылка = Объект.Ссылка;
	НС.Статус = Объект.Статус;
	НС.РСК_ВыдачаТСР = Объект.РСК_ВыдачаТСР;
	НС.Проведен = Истина;
	НС.РСК_ОтветственныйЗаОтгрузку = Объект.РСК_ОтветственныйЗаОтгрузку;
	НС.Склад = Объект.Склад;
	Парам.Вставить("Задание", Задание);
	
	Маршрут = Объект.Маршрут.Выгрузить();
	Маршрут.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("ДокументСсылка.ЗаданиеНаПеревозку"));
	Маршрут.ЗаполнитьЗначения(Объект.Ссылка, "Ссылка");
	Парам.Вставить("Маршрут", Маршрут);
	
	Распоряжения = Объект.Распоряжения.Выгрузить();
	Распоряжения.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("ДокументСсылка.ЗаданиеНаПеревозку"));
	Распоряжения.ЗаполнитьЗначения(Объект.Ссылка, "Ссылка");
	Парам.Вставить("Распоряжения", Распоряжения);
	
	ТаблицыСверкиСтатусовРО = Документы.ЗаданиеНаПеревозку.ПолучитьТаблицыСверкиСтатусовРО(Парам);
	МодульОбъекта = РеквизитФормыВЗначение("Объект");
	МодульОбъекта.СообщитьОбОшибкахСтатуса(ТаблицыСверкиСтатусовРО, Отказ);

КонецПроцедуры // ВыполнитьПроверкуПриСменеСтатусаНаСервере()

&НаСервере
&ИзменениеИКонтроль("ПоместитьРаспоряженияВХранилище")
Функция РСК_ПоместитьРаспоряженияВХранилище(КлючСвязи)
	...
	//Добавление колонки "Сборка завершена"
	Распоряжения.Колонки.Добавить("СборкаЗавершена", Новый ОписаниеТипов("Булево"));
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТоварыКОтгрузкеОстатки.ДокументОтгрузки КАК Распоряжение,
		|	ТоварыКОтгрузкеОстатки.КОтгрузкеОстаток КАК НеСобрано
		|ПОМЕСТИТЬ Сбор
		|ИЗ
		|	РегистрНакопления.ТоварыКОтгрузке.Остатки(, ДокументОтгрузки В (&Распоряжения)) КАК ТоварыКОтгрузкеОстатки
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ТоварыКОтгрузкеОбороты.ДокументОтгрузки,
		|	-ТоварыКОтгрузкеОбороты.СобраноПриход
		|ИЗ
		|	РегистрНакопления.ТоварыКОтгрузке.Обороты(, , , ДокументОтгрузки В (&Распоряжения)) КАК ТоварыКОтгрузкеОбороты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Сбор.Распоряжение КАК Распоряжение,
		|	СУММА(Сбор.НеСобрано) КАК НеСобрано
		|ИЗ
		|	Сбор КАК Сбор
		|
		|СГРУППИРОВАТЬ ПО
	...
	Для Каждого Стр Из Распоряжения Цикл
		Стр.СборкаЗавершена = Истина;
		Выборка.Сбросить();
		Если Выборка.НайтиСледующий(Новый Структура("Распоряжение", Стр.Распоряжение)) Тогда
			Стр.СборкаЗавершена = Выборка.НеСобрано <= 0;
		КонецЕсли;
	КонецЦикла;	
	//-- КонецТикета 25061	
	#КонецВставки
...
&НаКлиенте
Процедура РСК_ПечатьСертификатовВсеПосле(Команда)
	
	МассивОбъектов = Новый Массив;
	Для Каждого ДанныеСтроки Из Объект.Распоряжения Цикл
		Если ЗначениеЗаполнено(ДанныеСтроки.РСК_Накладная) Тогда
			Если ТипЗнч(ДанныеСтроки.РСК_Накладная) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
				Если МассивОбъектов.Найти(ДанныеСтроки.РСК_Накладная) = Неопределено Тогда 
					МассивОбъектов.Добавить(ДанныеСтроки.РСК_Накладная);
				КонецЕсли;  
			КонецЕсли;
		КонецЕсли;		
	КонецЦикла;
...
&НаКлиенте
Процедура РСК_ПечатьСертификатовВыделенныеПосле(Команда)
...
&НаКлиенте
Процедура ЗадатьВопросПечатьСертификатов(РежимПечати) 
	
	Если РСК_ВызовСервера.ВРаспоряженияхУказанаПечатьСертификатов(ПолучитьРаспоряженияДокумента()) Тогда
		ОбработкаОтвета = Новый ОписаниеОповещения("ОбработатьПечатьПакетаДокументов", ЭтотОбъект, Новый Структура("РежимПечати", РежимПечати));
		ПоказатьВопрос(ОбработкаОтвета, "Печатать сертификаты номенклатуры?", РежимДиалогаВопрос.ДаНет); 
	Иначе
		Если РежимПечати = "Все накладные" Тогда
			РСК_ПечатьПолногоКомплектаДокументов(Ложь);			
		ИначеЕсли РежимПечати = "Только выделенные" Тогда
			РСК_ПечатьКомплектаДокументовВыделенные(Ложь);	
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция ПолучитьРаспоряженияДокумента()
	
	МассивРаспоряжений = Объект.Распоряжения.Выгрузить().ВыгрузитьКолонку("Распоряжение");
...
&НаКлиенте
Процедура ОбработатьПечатьПакетаДокументов(Ответ, ДополнительныеПараметры) Экспорт
	
	Если Ответ <> Неопределено Тогда
		
		ПечататьСертификаты = Ложь;
		Если Ответ = КодВозвратаДиалога.Да Тогда
			ПечататьСертификаты = Истина;					
		КонецЕсли;
		
		Если ДополнительныеПараметры.РежимПечати = "Все накладные" Тогда
			РСК_ПечатьПолногоКомплектаДокументов(ПечататьСертификаты);			
		ИначеЕсли ДополнительныеПараметры.РежимПечати = "Только выделенные" Тогда
			РСК_ПечатьКомплектаДокументовВыделенные(ПечататьСертификаты);	
		КонецЕсли;
...
&НаКлиенте
Процедура РСК_ПечатьПолногоКомплектаДокументов(ПечатьСертификатов)
	...
	Для Каждого ДанныеСтроки Из Объект.Распоряжения Цикл
		Если ЗначениеЗаполнено(ДанныеСтроки.РСК_Накладная) Тогда
			...
			//++ РС Консалт: Трофимов Евгений 10.04.2023 Тикет 25062
			//e1cib/data/Документ.Задание?ref=94371d4af0b4403c43fef0b1ff172c4a
			ИначеЕсли ТипЗнч(ДанныеСтроки.РСК_Накладная) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
				ОбъектыПечати.Добавить(ДанныеСтроки.РСК_Накладная);
			//-- КонецТикета 25062				
	...
	ПечатьКомплектаДокументовЗавершение(СписокРеализаций, ЮрЛица, ФизЛица, ПечатьСертификатов);
	//++РС Консалт: Минаков А.С. Задача 21223
	...
	Если ОбъектыПечати.Количество() > 0 Тогда
		...
		УправлениеПечатьюУТКлиент.ПечатьКомплектаДокументов(ПараметрыПечати);
	КонецЕсли;
	//-- КонецТикета 25062		
...
&НаКлиенте
Процедура РСК_ПечатьКомплектаДокументовВыделенные(ПечатьСертификатов)
...
&НаКлиенте
&После("ПослеЗаписи")
Процедура РСК_ПослеЗаписи(ПараметрыЗаписи)
	
	//++РС Консалт: Минаков А.С. Задача 25303
	Если Не Объект.РСК_ВыдачаТСР 
		И Объект.Проведен
		И (Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаданийНаПеревозку.Отправлено")
		Или Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаданийНаПеревозку.Закрыто")) Тогда
		
		НакладныеКПереводуСтатуса = Новый Массив;
		
		Для Каждого Строка Из Объект.Распоряжения Цикл
			
			Если ТипЗнч(Строка.Распоряжение) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
				Накладная = Строка.Распоряжение
			ИначеЕсли ТипЗнч(Строка.РСК_Накладная) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
				Накладная = Строка.РСК_Накладная
			Иначе
				Продолжить
			КонецЕсли;
			
			РеквизитыНакладной = ОбщегоНазначенияУТВызовСервера.ЗначенияРеквизитовОбъекта(Накладная, "СкладПолучатель, Статус");
			Если Не РеквизитыНакладной.Статус = ПредопределенноеЗначение("Перечисление.СтатусыПеремещенийТоваров.Принято")
				И РСК_УправлениеСвойствамиВызовСервера.ЗначениеСвойства(РеквизитыНакладной.СкладПолучатель, "РС_СкладыСоисполнения") = Истина
				И НакладныеКПереводуСтатуса.Найти(Накладная) = Неопределено Тогда
				
				НакладныеКПереводуСтатуса.Добавить(Накладная)
			КонецЕсли
			
		КонецЦикла;
		
		Если НакладныеКПереводуСтатуса.Количество() Тогда
			
			ДополнительныеПараметры = НакладныеКПереводуСтатуса;
			ОписаниеОповещения = Новый ОписаниеОповещения("Вопрос_ПереводСтатусаНакладныхЗавершение", ЭтаФорма, ДополнительныеПараметры);
			ТекстВопроса = "Накладные - Перемещения на склад соисполнителя будут установлены как Принятые" + Символы.ПС + "Изменить статус накладных?";
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да, "Подтверждение изменения статуса накладных") 
...
&НаКлиенте
Процедура Вопрос_ПереводСтатусаНакладныхЗавершение(Ответ, МассивДокументов) Экспорт
	...
	ИзменитьСтатусПривилегировано(МассивДокументов)
	//++РС Консалт: Минаков А.С. Задача 25303
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ИзменитьСтатусПривилегировано(МассивДокументов)
	
	//++РС Консалт: Минаков А.С. Задача 25303	
	УстановитьПривилегированныйРежим(Истина);
	
	ОбщегоНазначенияУТВызовСервера.УстановитьСтатусДокументов(МассивДокументов, "Принято")
	//++РС Консалт: Минаков А.С. Задача 25303
