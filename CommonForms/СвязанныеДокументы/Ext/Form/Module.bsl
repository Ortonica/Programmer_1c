...
Функция РасшРС_ОбъектыПоКритериюОтбора(ЗначениеКритерияОтбора)
	...
	//++ РС Консалт: Трофимов Евгений 01.03.2023 Тикет 23958
	//e1cib/data/Документ.Задание?ref=bf4e94bb9c3a847f4cf0d217e8c3fd4b
	//Отказ от лишних типовых связей. Задача Битрикс 249627 
	Если ТипЗнч(Результат) = Тип("ТаблицаЗначений") Тогда
		УдаляемыеСтроки = Новый Массив;
		Для Каждого Стр Из Результат Цикл
			Если ТипЗнч(ЗначениеКритерияОтбора) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
				Если ТипЗнч(Стр.Ссылка) = Тип("ДокументСсылка.ПоручениеЭкспедитору") Тогда
					УдаляемыеСтроки.Добавить(Стр);
				КонецЕсли;
			ИначеЕсли ТипЗнч(ЗначениеКритерияОтбора) = Тип("ДокументСсылка.ЗаданиеНаПеревозку") Тогда
				Если ТипЗнч(Стр.Ссылка) = Тип("ДокументСсылка.РасходныйОрдерНаТовары") Тогда
					УдаляемыеСтроки.Добавить(Стр);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
