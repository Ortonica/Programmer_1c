...
//++ Конарев И.И. Печать сертификатов номенклатуры по правилам
					...
					Пока ВыборкаПоПриоритету.Следующий() Цикл
						...
						Пока Выборка.Следующий() Цикл
							...
							Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.ПрисоединенныйФайл,"Расширение") = "pdf" Тогда  
								РСК_Сервер.ПрисоединитьPDFКТабличномуДокументу(ТабличныйДокумент, Макет, Выборка.ПрисоединенныйФайл,Выборка.ТипСертификата);
							Иначе
							#КонецВставки
...
//Печать сертификатов номенклатуры из документа "ОтборРазмещениеТоваров" 
//
Функция СформироватьСформироватьПечатнуюФормуИзображенияСертификатовИзДокументовОтборРазмещение(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|	ОтборРазмещениеТоваровТоварыОтбор.Ссылка.Распоряжение КАК Ордер,
	|	ОтборРазмещениеТоваровТоварыОтбор.НомерСтроки КАК НомерСтроки
	|ПОМЕСТИТЬ ВТ_ТоварыПоОрдеру
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров.ТоварыОтбор КАК ОтборРазмещениеТоваровТоварыОтбор
	|ГДЕ
	|	ОтборРазмещениеТоваровТоварыОтбор.Ссылка.Распоряжение ССЫЛКА Документ.РасходныйОрдерНаТовары
	|	И ОтборРазмещениеТоваровТоварыОтбор.Ссылка В(&МассивОбъектов)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТоварыПоОрдеру.Номенклатура.ВидНоменклатуры КАК ВидНоменклатуры,
	|	ВТ_ТоварыПоОрдеру.Номенклатура КАК Номенклатура,
	|	ВТ_ТоварыПоОрдеру.Серия КАК Серия,
	|	ВТ_ТоварыПоОрдеру.Характеристика КАК Характеристика,
	|	РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Распоряжение КАК Распоряжение,
	|	ВТ_ТоварыПоОрдеру.НомерСтроки КАК НомерСтроки
	|ПОМЕСТИТЬ ВТ_ТоварыНаПечать
	|ИЗ
	|	ВТ_ТоварыПоОрдеру КАК ВТ_ТоварыПоОрдеру
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РасходныйОрдерНаТовары.ТоварыПоРаспоряжениям КАК РасходныйОрдерНаТоварыТоварыПоРаспоряжениям
	|		ПО ВТ_ТоварыПоОрдеру.Номенклатура = РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Номенклатура
	|			И ВТ_ТоварыПоОрдеру.Характеристика = РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Характеристика
	|			И ВТ_ТоварыПоОрдеру.Серия = РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Серия
	|			И ВТ_ТоварыПоОрдеру.Ордер = РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Ссылка
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТоварыНаПечать.НомерСтроки КАК НомерСтроки,
	|	ВТ_ТоварыНаПечать.Номенклатура КАК Номенклатура,
	|	ВТ_ТоварыНаПечать.Серия КАК Серия,
	|	ВТ_ТоварыНаПечать.Характеристика КАК Характеристика,
	|	ВТ_ТоварыНаПечать.Распоряжение КАК Распоряжение,
	...
	|ПОМЕСТИТЬ СертфиикатыНоменклатуры
	|ИЗ
	|	ВТ_ТоварыНаПечать КАК ВТ_ТоварыНаПечать
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОбластиДействияСертификатовНоменклатуры КАК ОбластиДействияСертификатов
	|		ПО (ВТ_ТоварыНаПечать.ВидНоменклатуры = ОбластиДействияСертификатов.ВидНоменклатуры
	|					И ВТ_ТоварыНаПечать.Номенклатура = ОбластиДействияСертификатов.Номенклатура
	|					И ВТ_ТоварыНаПечать.Характеристика = ОбластиДействияСертификатов.Характеристика
	|					И ВТ_ТоварыНаПечать.Серия = ОбластиДействияСертификатов.Серия
	|					И ОбластиДействияСертификатов.СертификатНоменклатуры.ДатаНачалаСрокаДействия <= ВТ_ТоварыНаПечать.Распоряжение.Дата
	|					И (ОбластиДействияСертификатов.СертификатНоменклатуры.ДатаОкончанияСрокаДействия >= ВТ_ТоварыНаПечать.Распоряжение.Дата
	|						ИЛИ ОбластиДействияСертификатов.СертификатНоменклатуры.ДатаОкончанияСрокаДействия = ДАТАВРЕМЯ(1, 1, 1))
	|				ИЛИ ВТ_ТоварыНаПечать.ВидНоменклатуры = ОбластиДействияСертификатов.ВидНоменклатуры
	|					И ВТ_ТоварыНаПечать.Номенклатура = ОбластиДействияСертификатов.Номенклатура
	|					И ВТ_ТоварыНаПечать.Характеристика = ОбластиДействияСертификатов.Характеристика
	...
	|ПОМЕСТИТЬ СертификатыНоменклатурыПрисоединенныеФайлы
	|ИЗ
	|	СертфиикатыНоменклатуры КАК СертфиикатыНоменклатуры
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СертификатыНоменклатурыПрисоединенныеФайлы КАК СертификатыНоменклатурыПрисоединенныеФайлы
	|		ПО СертфиикатыНоменклатуры.СертификатНоменклатуры = СертификатыНоменклатурыПрисоединенныеФайлы.ВладелецФайла
	|ГДЕ
	|	НЕ СертификатыНоменклатурыПрисоединенныеФайлы.ПометкаУдаления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СертфиикатыНоменклатуры.Распоряжение КАК Распоряжение,
	|	СертфиикатыНоменклатуры.НомерСтроки КАК НомерСтроки,
	|	СертфиикатыНоменклатуры.Номенклатура КАК Номенклатура,
	|	СертфиикатыНоменклатуры.Характеристика КАК Характеристика,
	|	СертфиикатыНоменклатуры.Серия КАК Серия,
	|	СертфиикатыНоменклатуры.СертификатНоменклатуры КАК СертификатНоменклатуры,
	|	СертфиикатыНоменклатуры.ТипСертификата КАК ТипСертификата,
	|	СертификатыНоменклатурыПрисоединенныеФайлы.ПрисоединенныйФайл КАК ПрисоединенныйФайл,
	|	ВЫБОР
	|		КОГДА СертификатыНоменклатурыПрисоединенныеФайлы.ПрисоединенныйФайл ЕСТЬ NULL
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК НетФайлов
	|ИЗ
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		ПервыйДокумент    = Ложь;
		НомерСтрокиНачало = ТабличныйДокумент.ВысотаТаблицы + 1;
		Подразделение     = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыборкаПоДокументам.Распоряжение, "Подразделение");
		...
		Пока ВыборкаПоНомеруСтроки.Следующий() Цикл
			...
			Пока ВыборкаПоТипуСертификата.Следующий() Цикл
				...
				Выборка = ВыборкаПоТипуСертификата.Выбрать();
				Пока Выборка.Следующий() Цикл 
					...
					Если НЕ НеобходимаПечатьСертификата(Неопределено, Выборка.ТипСертификата,Выборка.Номенклатура, 
															Выборка.Характеристика, Выборка.Серия, Выборка.Распоряжение) Тогда
						Продолжить;		
					КонецЕсли;
...
Функция НеобходимаПечатьСертификата(Документ, ТипСертификата, Номенклатура, Характеристика, Серия, ЗаказОснование = Неопределено)
